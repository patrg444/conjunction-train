#!/bin/bash
# Advanced continuous monitoring script for TCN large model training
...
display_training_summary() {
    ...
    if [[ -n "$start" ]]; then
        local start_seconds=$(date -j -f "%Y-%m-%d %H:%M:%S" "$start" +%s 2>/dev/null || echo "0")
...
check_progress_stall() {
    ...
    local last_timestamp_seconds=$(date -j -f "%Y-%m-%d %H:%M:%S" "$last_timestamp" +%s 2>/dev/null || echo "0")

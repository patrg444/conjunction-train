#!/bin/bash
# Ultimate direct fix using direct file creation approach

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77"
REMOTE_PATH="/home/ubuntu/audio_emotion/fixed_wav2vec_final.py"

echo "===== Creating Ultimate Wav2Vec Fix ====="

# Create a direct fix script to run on server by creating a new file
cat > direct_final_fix.sh << 'EOF'
#!/bin/bash
# Create a completely new fixed script file with proper commas

# Path to create the new fixed script
SCRIPT_PATH="/home/ubuntu/audio_emotion/fixed_wav2vec_final.py"

# Create a fresh fixed script file on the server
cat > "$SCRIPT_PATH" << 'SCRIPT_CONTENT'
#!/usr/bin/env python3
"""
Fixed WAV2VEC solution that:
...
chmod +x "$SCRIPT_PATH"
echo "Fixed script created at $SCRIPT_PATH"
EOF

# Upload the fix script
chmod +x direct_final_fix.sh
scp -i $KEY_PATH direct_final_fix.sh $SERVER:/tmp/

# Execute the fix script on the server
echo "Creating fixed script on server..."
ssh -i $KEY_PATH $SERVER "chmod +x /tmp/direct_final_fix.sh && /tmp/direct_final_fix.sh"
...
echo "Starting training with the completely new fixed script..."
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="/home/ubuntu/audio_emotion/wav2vec_ultimate_fixed_${TIMESTAMP}.log"
ssh -i $KEY_PATH $SERVER "cd /home/ubuntu/audio_emotion && nohup python $REMOTE_PATH > $LOG_FILE 2>&1 &"

# Create monitoring script
cat > monitor_ultimate_fix.sh << EOF
#!/bin/bash
...
chmod +x monitor_ultimate_fix.sh

# Create a script for downloading the model
cat > download_ultimate_model.sh << EOF
#!/bin/bash
# Script to download the model trained with the ultimate fixed solution

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77"
REMOTE_CHECKPOINT_DIR="/home/ubuntu/audio_emotion/checkpoints"
LOCAL_CHECKPOINT_DIR="./checkpoints_wav2vec_ultimate"

echo "===== Downloading Ultimate Wav2Vec Model ====="
...
chmod +x download_ultimate_model.sh

# Clean up
rm direct_final_fix.sh

echo "===== Ultimate Fix Applied and Training Restarted ====="
echo "A brand new fix script with proper commas has been created on the server."
echo "Training has been restarted with the completely new fixed script."
echo "To monitor the training progress, run: ./monitor_ultimate_fix.sh"
echo "To download the model when training is complete, run: ./download_ultimate_model.sh"

#!/bin/bash
# Download the trained wav2vec model that maps 'calm' to 'neutral'
...
# Find the best model weights (look for both patterns)
MODEL_PATH=$(ssh -i $KEY_PATH $EC2_HOST "ls -t $REMOTE_DIR/${MODEL_NAME}_*_best.weights.h5 $REMOTE_DIR/${MODEL_NAME}_best.weights.h5 2>/dev/null | head -n 1")

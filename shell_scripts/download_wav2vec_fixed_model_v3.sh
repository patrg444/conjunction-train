#!/bin/bash
# Download the wav2vec fixed model (v3) from EC2
# This version supports models trained with the new filename-based approach
...
LATEST_MODEL=$(ssh -i $KEY_PATH $EC2_HOST "cd $REMOTE_DIR/checkpoints && ls -t wav2vec_audio_only_fixed_v3_* 2>/dev/null | head -1 | xargs basename || echo ''")

if [ -z "$LATEST_MODEL" ]; then
  echo "No wav2vec_audio_only_fixed_v3 model found on the server."

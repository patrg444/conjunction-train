#!/bin/bash
# Direct SSH approach that fixes the issue directly

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77"

echo "===== Applying Direct SSH Fix ====="

# Directly run a command on the server
ssh -i $KEY_PATH $SERVER "cat > /home/ubuntu/audio_emotion/fixed_comma_direct.py" << 'EOF'
#!/usr/bin/env python3
"""
WAV2VEC audio emotion recognition model with fixed comma syntax.
"""
...
class WarmUpReduceLROnPlateau(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if epoch < self.warmup_epochs:
            ...
            # CRITICAL FIX: Added comma between learning_rate and warmup_lr
                ...
                if current < self.best:
                    ...
                    if self.wait >= self.patience:
                        ...
                        if old_lr > self.min_lr:
                            ...
                            # CRITICAL FIX: Added comma between learning_rate and new_lr
...
ssh -i $KEY_PATH $SERVER "chmod +x /home/ubuntu/audio_emotion/fixed_comma_direct.py"

# Start training with the new fixed script
echo "Starting training with directly created fixed script..."
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="/home/ubuntu/audio_emotion/wav2vec_direct_fixed_${TIMESTAMP}.log"
ssh -i $KEY_PATH $SERVER "cd /home/ubuntu/audio_emotion && nohup python fixed_comma_direct.py > $LOG_FILE 2>&1 &"

# Create monitoring script
cat > monitor_direct_fix.sh << EOF
#!/bin/bash
# Script to monitor the direct fixed version
...
PROCESS_COUNT=\$(ssh -i \$KEY_PATH \$SERVER "ps aux | grep 'python.*fixed_comma_direct.py' | grep -v grep | wc -l")
...
cat > download_direct_model.sh << EOF
#!/bin/bash
# Script to download the model trained with the direct fixed solution

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77"
REMOTE_CHECKPOINT_DIR="/home/ubuntu/audio_emotion/checkpoints"
LOCAL_CHECKPOINT_DIR="./checkpoints_wav2vec_direct"

echo "===== Downloading Direct Fixed Wav2Vec Model ====="
...
chmod +x download_direct_model.sh

echo "===== Direct SSH Fix Applied and Training Restarted ====="
echo "A completely new script with proper commas has been directly created on the server."
echo "Training has been restarted with this fixed script."
echo "To monitor the training progress, run: ./monitor_direct_fix.sh"
echo "To download the model when training is complete, run: ./download_direct_model.sh"

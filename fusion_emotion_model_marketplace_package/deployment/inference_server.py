...
# Initialize QAFlagEngine (config path relative to project root)
flagger = QAFlagEngine(str(ROOT_DIR / "configs" / "qa_flags_config.yaml"))
...
async def predict(
    ...
    try:
        ...
        # Compose turn_json for QAFlagEngine
        # For demo: treat all as "agent" and use only faceEmotion (extend as needed)
        turn_json = {
            "timestamp": "0",  # Placeholder; replace with real timestamp if available
            "speaker": "agent",
            "faceEmotion": result,
            "voiceTone": {},  # Fill if available
            "fusionScore": max(probs) if probs else 0.0,
        }
        enriched = flagger.process_turn(turn_json)

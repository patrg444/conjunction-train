...
# Create the remote training script - directly insert variables instead of using sed
cat > /tmp/remote_train.sh << EOF
#!/usr/bin/env bash
...
# Check if script was uploaded successfully
if ssh -i "$SSH_KEY" "$SSH_HOST" "ls -la ~/${REMOTE_SCRIPT}"; then
    echo "Script uploaded successfully."
else
    echo "Error: Failed to upload training script."
    exit 1
fi
...
# Set up a basic monitoring job to check status periodically
cat > monitor_g5_fixed_${TIMESTAMP}.sh << EOL
#!/usr/bin/env bash
...
ssh -i "\$SSH_KEY" "\$SSH_HOST" "tail -n 50 ~/${REMOTE_LOG}"
...
chmod +x monitor_g5_fixed_${TIMESTAMP}.sh
echo "Created monitoring script: monitor_g5_fixed_${TIMESTAMP}.sh"

#!/bin/bash
# Script to create and run a training script without matplotlib

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Creating a minimal training script without matplotlib dependency..."
...
# Create a training script that doesn't use matplotlib
cat > scripts/train_nompl.py << 'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Training script for the branched model with matplotlib dependency removed.
This version focuses just on training the model, not visualization.
"""
...
print("SIMPLE TRAINING SCRIPT STARTING - NO VISUALIZATION")
...
def train_model():
    ...
    # We skip plotting here - no matplotlib
...
chmod +x scripts/train_nompl.py

# Clear the current training log
> training.log

# Run the fixed training script using the proper conda environment
echo "Starting minimal training script without matplotlib..." > training.log
source $HOME/anaconda3/etc/profile.d/conda.sh
conda activate tensorflow2_p310
echo "Conda environment activated, running script..." >> training.log
cd ~/emotion_training
nohup python scripts/train_nompl.py >> training.log 2>&1 &
echo "Training restarted with no-matplotlib script. Check training.log for progress."
echo "You can run 'ps aux | grep python' to see if the process is running."
EOF

echo "Final script without matplotlib dependency applied."

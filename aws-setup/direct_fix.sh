#!/bin/bash
# Script to directly fix the training script on the server

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Creating a fixed script directly on the server..."
...
if [ ! -f "scripts/train_branched_6class.py" ]; then
  echo "Error: Original script not found!"
  exit 1
fi
...
cat > scripts/train_branched_6class_py2.py << 'PYEOF'
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Training script for the branched model with Python 2.7 compatibility.
This version has been modified to work with older Python versions.
"""
...
echo "Created compatibility-fixed training script..."
chmod +x scripts/train_branched_6class_py2.py

# Start the training with the fixed script
echo "Starting training with fixed script..." > training.log
nohup python scripts/train_branched_6class_py2.py >> training.log 2>&1 &
echo "Training process started. Check training.log for progress."
EOF

#!/bin/bash
# Monitor the branched model with focal loss training on EC2
...
LOG_FILE="/home/ec2-user/emotion_training/training_branched_focal_loss.log"

echo "======================================================"
echo "  MONITORING BRANCHED MODEL WITH FOCAL LOSS"
echo "======================================================"
...
PROCESS_INFO=$(ssh -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "ps aux | grep train_branched_focal_loss.py | grep -v grep")

if [ -z "$PROCESS_INFO" ]; then
    echo "Warning: Training process is not running"
    echo "To start the training, use: ./aws-setup/deploy_branched_focal_loss.sh"
    exit 1
fi

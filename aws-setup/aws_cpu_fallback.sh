#!/bin/bash
# Modified AWS Instance Fallback Script for CPU instances
# This script attempts to launch high-performance CPU instances in sequence
...
# Banner
echo -e "${BLUE}==========================================================================${NC}"
echo -e "${BLUE}              HIGH-PERFORMANCE CPU INSTANCE SETUP                         ${NC}"
echo -e "${BLUE}==========================================================================${NC}"
echo ""
...
# Define CPU instance types to try (in order of preference)
# These are all within your 96 vCPU limit
CPU_INSTANCES=(
    "c5.24xlarge"  # 96 vCPUs, high compute
    "c5.18xlarge"  # 72 vCPUs, high compute
    "c5.12xlarge"  # 48 vCPUs, high compute
    "m5.24xlarge"  # 96 vCPUs, balanced
    "m5.16xlarge"  # 64 vCPUs, balanced
    "r5.24xlarge"  # 96 vCPUs, high memory
    "r5.16xlarge"  # 64 vCPUs, high memory
    "r5.12xlarge"  # 48 vCPUs, high memory
    "m5.8xlarge"   # 32 vCPUs, balanced
)
...
for instance_type in "${CPU_INSTANCES[@]}"; do
...
if [ -z "$INSTANCE_ID" ]; then
    echo -e "${RED}Failed to launch any instance. Please check your AWS account limits or try again later.${NC}"
    exit 1
fi
...
#!/bin/bash
# CPU-specific optimizations for $INSTANCE_TYPE
...
echo -e "${RED}Hourly cost for $INSTANCE_TYPE varies between \$0.70 and \$4.50 depending on the instance type.${NC}"
echo ""

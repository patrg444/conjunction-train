#!/bin/bash
# Deploy and run the branched model with adjusted optimizer parameters

# Set up variables
INSTANCE_IP="3.235.76.0"
USERNAME="ec2-user"
KEY_FILE="../aws-setup/emotion-recognition-key-fixed-20250323090016.pem"
SCRIPT_NAME="train_branched_optimizer.py"
LOCAL_DIR="$(pwd)"
REMOTE_DIR="~/emotion_training"
LOG_FILE="training_branched_optimizer.log"

echo "==================================================="
echo "  DEPLOYING BRANCHED MODEL WITH ADJUSTED OPTIMIZER"
echo "==================================================="
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "cat > ${REMOTE_DIR}/run_branched_optimizer.sh << 'EOL'
#!/bin/bash
cd ~/emotion_training

# Run the optimizer script with Python 3.9
nohup /usr/bin/python3.9 scripts/${SCRIPT_NAME} > ${LOG_FILE} 2>&1 &
echo \$! > branched_optimizer_pid.txt
echo \"Training process started with PID \$(cat branched_optimizer_pid.txt)\"
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "chmod +x ${REMOTE_DIR}/run_branched_optimizer.sh"

# Run the script
echo "Starting training process on remote server..."
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "${REMOTE_DIR}/run_branched_optimizer.sh"

echo
echo "Deployment complete!"
echo "To monitor training, run: cd aws-setup && ./monitor_branched_optimizer.sh"
echo "==================================================="

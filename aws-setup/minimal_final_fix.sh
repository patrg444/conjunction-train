#!/bin/bash
# Script to create and run a truly minimal training script

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Creating a minimal no-dependency training script..."
...
cat > scripts/train_minimal.py << 'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Ultra minimal training script that avoids dependencies on sklearn.
"""
...
print("ULTRA MINIMAL TRAINING SCRIPT STARTING")
...
chmod +x scripts/train_minimal.py

# Clear the current training log
> training.log

# Run the fixed training script using the proper conda environment
echo "Starting ultra minimal training script..." > training.log
source $HOME/anaconda3/etc/profile.d/conda.sh
conda activate tensorflow2_p310
echo "Conda environment activated, running script..." >> training.log
cd ~/emotion_training
nohup python scripts/train_minimal.py >> training.log 2>&1 &
echo "Training restarted with minimal script. Check training.log for progress."
ps aux | grep python >> training.log
EOF

echo "Minimal training script applied."

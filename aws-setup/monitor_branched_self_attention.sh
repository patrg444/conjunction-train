#!/bin/bash
# Monitor the branched model with self-attention training on EC2
...
LOG_FILE="/home/ec2-user/emotion_training/training_branched_self_attention.log"

echo "======================================================"
echo "  MONITORING BRANCHED MODEL WITH SELF-ATTENTION"
echo "======================================================"
...
PROCESS_INFO=$(ssh -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "ps aux | grep train_branched_self_attention.py | grep -v grep")

if [ -z "$PROCESS_INFO" ]; then
    echo "Warning: Training process is not running"
    echo "To start the training, use: ./aws-setup/deploy_branched_self_attention.sh"
    exit 1
fi

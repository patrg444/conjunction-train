#!/bin/bash
# Script to fix and run the training script using the proper conda environment

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Applying fix and running with the proper conda environment..."
ssh -i "${KEY_FILE}" -o StrictHostKeyChecking=no ${USERNAME}@${INSTANCE_IP} << 'EOF'
cd ~/emotion_training

# Kill any previous training process
pkill -f "python.*train"

# Create a backup of the original script if not already done
if [ ! -f "scripts/train_branched_6class.py.backup" ]; then
  cp scripts/train_branched_6class.py scripts/train_branched_6class.py.backup
fi

# Add encoding declaration to the top of the file
cat > scripts/train_branched_6class_fixed.py << 'PYEOF'
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Training script for the branched model fixed with the proper encoding declaration.
"""
...
def train_model():
    ...
    print("Starting branched model training process...")
    ...
    if ravdess_audio is None or cremad_audio is None:
        print("Error: Failed to load data")
        return
    
    # Combine datasets
    all_audio = ravdess_audio + cremad_audio
    all_video = ravdess_video + cremad_video
    all_labels = np.vstack([ravdess_labels, cremad_labels])
    ...
    model_dir = "models/branched_6class"
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    
    # Define callbacks
    checkpoint_path = os.path.join(model_dir, "model_epoch_%02d_val_loss_%.4f.h5")
    callbacks = [
        ModelCheckpoint(
            checkpoint_path,
            monitor='val_loss',
    ...
    print("Training history plot saved to:", history_path)
...
echo "Created fixed training script with the proper encoding declaration..."
chmod +x scripts/train_branched_6class_fixed.py

# Clear the current training log
> training.log

# Run the fixed script using the TensorFlow conda environment
echo "Starting training with the fixed script using conda environment..." >> training.log
echo "Activating conda tensorflow environment and running the script..." >> training.log
source $HOME/anaconda3/etc/profile.d/conda.sh
conda activate tensorflow2_p310
conda env list >> training.log
python --version >> training.log
echo "NumPy and TensorFlow versions:" >> training.log
python -c "import numpy; print('NumPy:', numpy.__version__)" >> training.log
python -c "import tensorflow; print('TensorFlow:', tensorflow.__version__)" >> training.log

echo "Starting training script..." >> training.log
cd ~/emotion_training
nohup python scripts/train_branched_6class_fixed.py >> training.log 2>&1 &

echo "Training process started with conda environment. Check training.log for progress."
EOF

echo "Fix applied and training restarted with conda environment."

#!/bin/bash
# Deploy and run the branched model with L2 regularization

# Set up variables
INSTANCE_IP="3.235.76.0"
USERNAME="ec2-user"
KEY_FILE="../aws-setup/emotion-recognition-key-fixed-20250323090016.pem"
SCRIPT_NAME="train_branched_regularization.py"
LOCAL_DIR="$(pwd)"
REMOTE_DIR="~/emotion_training"
LOG_FILE="training_branched_regularization.log"

echo "==================================================="
echo "  DEPLOYING BRANCHED MODEL WITH L2 REGULARIZATION"
echo "==================================================="
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "cat > ${REMOTE_DIR}/run_branched_regularization.sh << 'EOL'
#!/bin/bash
cd ~/emotion_training

# Run the regularization script with Python 3.9
nohup /usr/bin/python3.9 scripts/${SCRIPT_NAME} > ${LOG_FILE} 2>&1 &
echo \$! > branched_regularization_pid.txt
echo \"Training process started with PID \$(cat branched_regularization_pid.txt)\"
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "chmod +x ${REMOTE_DIR}/run_branched_regularization.sh"

# Run the script
echo "Starting training process on remote server..."
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "${REMOTE_DIR}/run_branched_regularization.sh"

echo
echo "Deployment complete!"
echo "To monitor training, run: cd aws-setup && ./monitor_branched_regularization.sh"
echo "==================================================="

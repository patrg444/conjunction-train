...
EPOCHS=15  # Increased from 3 to 15 epochs
WEIGHT_DECAY=0.01
DROPOUT=0.1
SCHEDULER="cosine"
GRAD_CLIP=1.0
LOG_DIR="training_logs_humor"
EXP_NAME="${MODEL_NAME//\//_}_extended_training"
...
show_help() {
    ...
    echo "Train XLM-RoBERTa-large for humor detection on an EC2 G5.4xlarge instance with extended training (15 epochs)"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}Setting up XLM-RoBERTa-large extended training on EC2 G5.4xlarge instance...${NC}"
echo -e "${YELLOW}This training will run for $EPOCHS epochs${NC}"
...
REMOTE_SCRIPT="/home/ubuntu/train_xlm_roberta_large_extended.py"
...
echo -e "${BLUE}Starting XLM-RoBERTa-large extended training (${EPOCHS} epochs) on EC2...${NC}"
ssh -i "$KEY_PATH" ubuntu@$INSTANCE_IP << EOF
    ...
    echo "Starting extended training at \$(date)"
    python $REMOTE_SCRIPT \
        ...
        --exp_name $EXP_NAME \
        --gpus $GPUS \
        $FP16 > /home/ubuntu/$LOG_DIR/$EXP_NAME/training.log 2>&1 &

    echo "Training job submitted to EC2 G5.4xlarge instance!"
    echo "The process will run in the background."
    sleep 2
    ps aux | grep python | grep train_xlm_roberta_large_extended.py
EOF

echo -e "${GREEN}New extended training job (${EPOCHS} epochs) submitted to EC2 G5.4xlarge instance!${NC}"
echo -e "${YELLOW}To monitor the training progress, use:${NC}"
echo "  ./monitor_xlm_roberta_training.sh --exp_name ${EXP_NAME}"
echo ""
echo -e "${YELLOW}Or check the logs directly with:${NC}"
echo "  ssh -i $KEY_PATH ubuntu@$INSTANCE_IP 'tail -f /home/ubuntu/$LOG_DIR/$EXP_NAME/training.log'"
echo ""
echo -e "${YELLOW}To download the model after training:${NC}"
echo "  scp -r -i $KEY_PATH ubuntu@$INSTANCE_IP:/home/ubuntu/$LOG_DIR/$EXP_NAME/final_model ."

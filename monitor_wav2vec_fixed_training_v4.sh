#!/bin/bash
# Enhanced monitoring script for wav2vec emotion recognition training (v4)
...
if [ -z "$LOG_FILE" ]; then
  echo "No log file found matching pattern 'wav2vec_fixed_training_v4_*.log'"
  LOG_FILE="wav2vec_fixed_training_v4_$(date +"%Y%m%d")"_*.log
  echo "Will use default pattern: $LOG_FILE"
fi

echo "Enhanced Wav2Vec Training Monitor (v4)"
echo "================================================"
echo "Monitoring log file: $REMOTE_DIR/$LOG_FILE"

# Check if the training process is running
TRAINING_PID=$(ssh -i $KEY_PATH $EC2_HOST "pgrep -f 'python3.*train_wav2vec_audio_only_fixed_v4.py' || echo ''")

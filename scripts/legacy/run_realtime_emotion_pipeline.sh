#!/bin/bash
#
# Real-time Emotion Recognition with Original Pipeline
# This script launches the real-time emotion recognition application that processes
# webcam video and microphone audio using the same pipeline as our trained model.
...
echo -e "${BLUE}Real-time Emotion Recognition with Original Pipeline${NC}\n"
...
# Run the application
echo -e "\nStarting real-time emotion recognition with original pipeline..."
...
echo -e "- Predicts emotions in real-time using our trained model"
echo -e "- Applies smoothing to reduce jumpiness"

$PYTHON_PATH scripts/realtime_emotion_with_original_pipeline.py \
    --model "$MODEL_PATH" \

#!/usr/bin/env python3
"""
Enhanced training script for the multimodal emotion recognition model with branched LSTM architecture.
Includes stronger regularization techniques to combat overfitting:
- Increased L2 regularization on all layers
- Higher dropout rates
- Longer early stopping patience
- Modified learning rate scheduling
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("train_branched_enhanced.log"),
...
def define_branched_model(video_input_shape, audio_input_shape, num_classes, use_attention=True):
    """Defines the branched LSTM model architecture with stronger regularization.
    ...
    video_branch1 = LSTM(64, return_sequences=True, 
                         ...
                         name='video_branch1_lstm1')(video_input)
    video_branch1 = Dropout(dropout_rate)(video_branch1)
    video_branch1 = LSTM(32, 
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         recurrent_regularizer=regularizers.l2(recurrent_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
    ...
    video_branch2_seq = Bidirectional(
        LSTM(64, return_sequences=True,
             kernel_regularizer=regularizers.l2(kernel_l2),
             recurrent_regularizer=regularizers.l2(recurrent_l2),
             bias_regularizer=regularizers.l2(bias_l2)),
        name='video_branch2_bilstm1')(video_input)
    video_branch2_seq = Dropout(dropout_rate)(video_branch2_seq)
    ...
    audio_branch1 = LSTM(64, return_sequences=True, 
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         recurrent_regularizer=regularizers.l2(recurrent_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
                         name='audio_branch1_lstm1')(audio_input)
    audio_branch1 = Dropout(dropout_rate)(audio_branch1)
    audio_branch1 = LSTM(32, 
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         recurrent_regularizer=regularizers.l2(recurrent_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
    ...
    audio_branch2_seq = Bidirectional(
        LSTM(64, return_sequences=True, 
             kernel_regularizer=regularizers.l2(kernel_l2),
             recurrent_regularizer=regularizers.l2(recurrent_l2),
             bias_regularizer=regularizers.l2(bias_l2)),
        name='audio_branch2_bilstm1')(audio_input)
    audio_branch2_seq = Dropout(dropout_rate)(audio_branch2_seq)
    ...
    audio_branch3 = Conv1D(64, kernel_size=3, padding='same', activation='relu',
                          kernel_regularizer=regularizers.l2(kernel_l2),
                          bias_regularizer=regularizers.l2(bias_l2),
                          name='audio_branch3_conv1')(audio_input)
    audio_branch3 = MaxPooling1D(pool_size=2, name='audio_branch3_pool1')(audio_branch3)
    audio_branch3 = Dropout(dropout_rate)(audio_branch3)
    audio_branch3 = LSTM(32,
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         recurrent_regularizer=regularizers.l2(recurrent_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
                         name='audio_branch3_lstm')(audio_branch3)

    # ---- Video modality fusion ----
    video_fusion = concatenate([video_branch1, video_branch2], name='video_fusion')
    video_fusion = Dense(64, activation='relu', 
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
                         name='video_fusion_dense')(video_fusion)
    video_fusion = Dropout(dropout_rate)(video_fusion)

    # ---- Audio modality fusion ----
    audio_fusion = concatenate([audio_branch1, audio_branch2, audio_branch3], name='audio_fusion')
    audio_fusion = Dense(64, activation='relu',
                         kernel_regularizer=regularizers.l2(kernel_l2),
                         bias_regularizer=regularizers.l2(bias_l2),
                         name='audio_fusion_dense')(audio_fusion)
    audio_fusion = Dropout(dropout_rate)(audio_fusion)
    ...
    if use_attention:
        ...
        # Simple concatenation fusion if attention is disabled
        cross_modal_fusion = concatenate([video_fusion, audio_fusion], name='cross_modal_fusion_simple')
    ...
    dropout = Dropout(final_dropout_rate)(dense1)
    dense2 = Dense(64, activation='relu',
                   kernel_regularizer=regularizers.l2(kernel_l2),
                   bias_regularizer=regularizers.l2(bias_l2),
                   name='dense2')(dropout)
    dropout2 = Dropout(final_dropout_rate)(dense2)

    # Output layer
    output_layer = Dense(num_classes, activation='softmax', 
                         kernel_regularizer=regularizers.l2(kernel_l2/2),  # Lower regularization for output layer
                         bias_regularizer=regularizers.l2(bias_l2/2),
...
def compile_model(model, learning_rate=0.001):
    ...
    """
    # Add gradient clipping to optimizer to prevent exploding gradients
    optimizer = Adam(
...
def train_multimodal_model(
    ...
    epochs=100,
    batch_size=32,
    model_save_path='models/branched_enhanced/'
):
    ...
    """
    # Create callbacks with improved parameters
    early_stopping = EarlyStopping(
        monitor='val_loss',
        patience=20,  # Increased patience from 15 to 20
...
def evaluate_multimodal_model(model, X_test_video, X_test_audio, y_test, class_names=None, output_dir="model_evaluation/branched_enhanced"):
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Train the enhanced branched LSTM model for emotion recognition')
    parser.add_argument('--data_dir', type=str, default='processed_features_3_5s',
                        help='Directory containing processed features (default: processed_features_3_5s)')
    ...
    parser.add_argument('--model_dir', type=str, default='models/branched_enhanced/',
                        help='Directory to save model checkpoints (default: models/branched_enhanced/)')
    parser.add_argument('--eval_dir', type=str, default='model_evaluation/branched_enhanced/',
                        help='Directory to save evaluation results (default: model_evaluation/branched_enhanced/)')
    parser.add_argument('--no_attention', action='store_true',
                        help='Disable attention mechanisms to reduce model complexity')
    ...
    try:
        model = define_branched_model(video_input_shape, audio_input_shape, NUM_CLASSES, use_attention=USE_ATTENTION)
    ...
    # Z-score normalization for better training
    # Normalize each feature dimension separately
    # Only compute statistics from training data to avoid data leakage
    
    # Normalize video features
    video_mean = np.mean(X_train_video_padded, axis=(0, 1), keepdims=True)
    video_std = np.std(X_train_video_padded, axis=(0, 1), keepdims=True) + 1e-8  # Add epsilon to avoid division by zero
    
    X_train_video_padded = (X_train_video_padded - video_mean) / video_std
    X_val_video_padded = (X_val_video_padded - video_mean) / video_std
    X_test_video_padded = (X_test_video_padded - video_mean) / video_std
    
    # Normalize audio features
    audio_mean = np.mean(X_train_audio_padded, axis=(0, 1), keepdims=True)
    audio_std = np.std(X_train_audio_padded, axis=(0, 1), keepdims=True) + 1e-8
    
    X_train_audio_padded = (X_train_audio_padded - audio_mean) / audio_std
    X_val_audio_padded = (X_val_audio_padded - audio_mean) / audio_std
    X_test_audio_padded = (X_test_audio_padded - audio_mean) / audio_std
    
    logging.info("Feature normalization applied to both modalities")
    ...
    try:
        ...
        final_model_path = os.path.join(MODEL_SAVE_PATH, 'final_branched_model_enhanced.h5')

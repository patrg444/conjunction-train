...
enhanced with a self-attention mechanism after the LSTM block.
"""
...
from tensorflow.keras.layers import Masking, BatchNormalization, LayerNormalization, MultiHeadAttention # Added MultiHeadAttention
from tensorflow.keras.layers import GlobalAveragePooling1D, GlobalMaxPooling1D, Add # Added Add
...
print("EARLY FUSION (AUDIO POOLING) HYBRID MODEL WITH LSTM + ATTENTION") # Updated Title
...
def create_audio_pooling_lstm_attention_model(combined_feature_dim): # Renamed function
    """
    ...
    """
    print("Creating early-fusion LSTM model (Audio Pooling) with Attention:") # Updated description
    ...
    # --- Shared Sequence Processing (LSTM/BiLSTM + Attention) ---
    ...
    # Second BiLSTM layer, ensure it returns sequences for attention
    features_before_attention = Bidirectional(LSTM(
        LSTM_UNITS[1], return_sequences=True, dropout=0.3, recurrent_dropout=0.2,
        kernel_regularizer=l2(L2_REGULARIZATION), recurrent_regularizer=l2(L2_REGULARIZATION/2),
        kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT), recurrent_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    ))(x)
    features_before_attention = Dropout(0.3)(features_before_attention) # Apply dropout
    ...
    )(query=features_before_attention, value=features_before_attention, key=features_before_attention)
    # Add & Norm (Residual connection)
    x = LayerNormalization()(Add()([features_before_attention, attention_output]))

    # Pool the sequence output after attention
...
# --- Training Function ---
def train_model():
    """Main function to train the audio pooling LSTM model with attention."""
    print("Starting Audio Pooling LSTM + Attention model training...") # Updated print
    ...
    model = create_audio_pooling_lstm_attention_model(combined_feature_dim) # Updated function call
    model.summary()

    # Define callbacks
    checkpoint_filename = 'best_model_audio_pooling_lstm_attention.keras' # New checkpoint name

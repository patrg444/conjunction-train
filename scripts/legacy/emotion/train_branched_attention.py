...
# -*- coding: utf-8 -*-
"""
Enhanced training script with temporal attention, focal loss, and audio augmentation.
...
AUGMENTATION_FACTOR = 2  # How many augmented samples to create per original sample
...
def process_data(path_pattern, max_files=None, augment=False):
    ...
    for file_path in files:
        try:
            ...
            if augment:
                # Only augment audio for now
                for _ in range(AUGMENTATION_FACTOR - 1):  # -1 because we already have the original
                    aug_audio = augment_audio(audio_features)
                    audio_data.append(aug_audio)
                    video_data.append(video_features)  # Use same video
                    labels.append(label)  # Same label
            
        except Exception as e:
            print("Error processing file %s: %s" % (file_path, str(e)))
            skipped += 1
    
    print("Processed %d files, skipped %d files" % (len(audio_data) // AUGMENTATION_FACTOR, skipped))
    if augment:
        print("Created %d total samples with augmentation" % len(audio_data))
...
def train_model():
    """Main function to train the model with attention, focal loss, and augmentation."""
    print("Starting enhanced model training with ATTENTION and FOCAL LOSS...")
    
    # Process RAVDESS data - note the nested pattern to find files in actor folders
    ravdess_pattern = "ravdess_features_facenet/*/*.npz"
    ravdess_audio, ravdess_video, ravdess_labels = process_data(ravdess_pattern, augment=True)
    
    # Process CREMA-D data
    cremad_pattern = "crema_d_features_facenet/*.npz"
    cremad_audio, cremad_video, cremad_labels = process_data(cremad_pattern, augment=True)
    ...
    print("Dataset size (with augmentation):")
    ...
    model_dir = "models/attention_focal_loss"

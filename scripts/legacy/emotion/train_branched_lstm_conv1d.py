...
# -*- coding: utf-8 -*-
"""
Branched hybrid model using Conv1D for audio and LSTM/BiLSTM for video.
- No TCN.
- No Attention.
- Branched fusion.
"""
...
# WEIGHT_DECAY = 0.0005 # Adam optimizer doesn't directly use weight_decay

# Model specific parameters
AUDIO_CONV_FILTERS = [64, 128]
AUDIO_DENSE_UNITS = 64 # Dense layer after audio conv pooling
VIDEO_LSTM_UNITS = [128, 64] # LSTM units for video branch
MERGED_DENSE_UNITS = [256, 128]

print("BRANCHED HYBRID MODEL WITH CONV1D (AUDIO) AND LSTM (VIDEO)")
...
# Removed attention_block and residual_tcn_block functions

def create_branched_lstm_conv1d_model(audio_feature_dim, video_feature_dim):
    """
    Create a branched hybrid model using Conv1D for audio and LSTM/BiLSTM for video.
    ...
    """
    print("Creating branched hybrid model with Conv1D (Audio) and LSTM (Video):")
    ...
    print(f"- Audio Dense Units: {AUDIO_DENSE_UNITS}")
    ...
    # Skip connection around second conv layer (optional, kept from original)
    audio_skip = Conv1D(
        AUDIO_CONV_FILTERS[1], kernel_size=1, padding='same',
    ...
    audio_x = Dropout(0.4)(audio_x) # Final audio branch output
    ...
    video_x = Bidirectional(LSTM(
        VIDEO_LSTM_UNITS[1], return_sequences=False, dropout=0.3, recurrent_dropout=0.2, # Last LSTM returns single vector
        kernel_regularizer=l2(L2_REGULARIZATION), recurrent_regularizer=l2(L2_REGULARIZATION/2),
        kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT), recurrent_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    ))(video_x)
    # Output shape: (batch_size, VIDEO_LSTM_UNITS[1] * 2)

    # Dense layer for video features
    video_x = Dense(
        VIDEO_LSTM_UNITS[0], activation='relu', # Project to a consistent dimension
        kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    )(video_x)
    video_x = LayerNormalization()(video_x)
    video_x = Dropout(0.4)(video_x) # Final video branch output

    # --- Fusion (Branched Style) ---
    # Project each branch output before concatenating (kept from original)
    audio_projection = Dense(
        MERGED_DENSE_UNITS[0]//2, # Half of the first merged dense layer size
    ...
    # First merged dense layer (removed skip connection around it for simplicity)
    merged = Dense(
...
# --- Training Function (Updated model creation call and checkpoint name) ---
def train_model():
    """Main function to train the branched LSTM/Conv1D model."""
    print("Starting branched LSTM/Conv1D model training...")
    print(f"- Using learning rate: {LEARNING_RATE}")

    ravdess_pattern = "data/ravdess_features_facenet/*/*.npz" # Added data/ prefix
    ravdess_audio, ravdess_video, ravdess_labels = process_data(ravdess_pattern)
    cremad_pattern = "data/crema_d_features_facenet/*.npz" # Added data/ prefix
    ...
    model = create_branched_lstm_conv1d_model(audio_feature_dim, video_feature_dim) # Updated function call
    model.summary()

    # Define callbacks (updated checkpoint name)
    checkpoint_filename = 'best_model_branched_lstm_conv1d.keras' # New checkpoint name

    # --- Load weights if checkpoint exists ---
    if os.path.exists(checkpoint_filename):
        print(f"Checkpoint file found: {checkpoint_filename}. Loading weights...")
        try:

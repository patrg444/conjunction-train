...
enhanced with self-attention mechanisms in both branches.
"""
...
AUDIO_DENSE_UNITS = 64
VIDEO_LSTM_UNITS = [128, 64]
MERGED_DENSE_UNITS = [256, 128]
ATTENTION_HEADS = 4 # Number of attention heads
ATTENTION_KEY_DIM = 64 # Key dimension for attention

print("NON-BRANCHED HYBRID MODEL WITH CONV1D (AUDIO) + LSTM (VIDEO) + SELF-ATTENTION")
...
def create_lstm_conv1d_attention_model(audio_feature_dim, video_feature_dim):
    """
    Create a non-branched hybrid model using Conv1D for audio and LSTM/BiLSTM for video,
    with added self-attention layers.
    ...
    """
    print("Creating non-branched hybrid model with Conv1D+Attention (Audio) and LSTM+Attention (Video):")
    ...
    audio_x = BatchNormalization()(audio_x)
    audio_features_before_attention = SpatialDropout1D(0.2)(audio_x) # Output shape: (batch, time_steps, features)

    # Audio Self-Attention
    audio_attention_output = MultiHeadAttention(
        num_heads=ATTENTION_HEADS, key_dim=ATTENTION_KEY_DIM, dropout=0.1,
        kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    )(query=audio_features_before_attention, value=audio_features_before_attention, key=audio_features_before_attention)
    # Add & Norm (Residual connection)
    audio_x = LayerNormalization()(Add()([audio_features_before_attention, audio_attention_output]))
    ...
    # --- Video Branch (LSTM/BiLSTM + Attention) ---
    ...
    video_features_before_attention = Bidirectional(LSTM(
        VIDEO_LSTM_UNITS[1], return_sequences=True, dropout=0.3, recurrent_dropout=0.2, # Keep return_sequences=True for attention
        kernel_regularizer=l2(L2_REGULARIZATION), recurrent_regularizer=l2(L2_REGULARIZATION/2),
        kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT), recurrent_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    ))(video_x)
    video_features_before_attention = Dropout(0.3)(video_features_before_attention) # Apply dropout

    # Video Self-Attention
    video_attention_output = MultiHeadAttention(
        num_heads=ATTENTION_HEADS, key_dim=ATTENTION_KEY_DIM, dropout=0.1,
        kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT)
    )(query=video_features_before_attention, value=video_features_before_attention, key=video_features_before_attention)
    # Add & Norm (Residual connection)
    video_x = LayerNormalization()(Add()([video_features_before_attention, video_attention_output]))

    # Pool after attention
    video_x = GlobalMaxPooling1D()(video_x) # Pool the sequence output from attention

    # Dense layer for video features
    video_x = Dense(
...
def train_model():
    """Main function to train the LSTM/Conv1D model with attention."""
    print("Starting LSTM/Conv1D + Attention model training...") # Updated print
    ...
    model = create_lstm_conv1d_attention_model(audio_feature_dim, video_feature_dim) # Updated function call
    model.summary()

    # Define callbacks (updated checkpoint name)
    checkpoint_filename = 'best_model_lstm_conv1d_attention.keras' # New checkpoint name

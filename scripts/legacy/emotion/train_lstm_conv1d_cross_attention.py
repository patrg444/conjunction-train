...
enhanced with self-attention within each branch and cross-modal attention
between branches before fusion.
"""
...
from tensorflow.keras.layers import Activation, Add, SpatialDropout1D, LayerNormalization, MultiHeadAttention # Added MultiHeadAttention
...
import glob
import random
# Assuming these generators are in the same directory or accessible via PYTHONPATH
...
AUDIO_DENSE_UNITS = 64 # Used after pooling
VIDEO_LSTM_UNITS = [128, 64]
MERGED_DENSE_UNITS = [256, 128]
ATTENTION_HEADS = 4 # Number of attention heads (used for both self and cross)
ATTENTION_KEY_DIM = 64 # Key dimension for attention (used for both self and cross)

print("NON-BRANCHED HYBRID MODEL WITH CONV1D (AUDIO) + LSTM (VIDEO) + SELF & CROSS-ATTENTION") # Updated Title
...
def create_lstm_conv1d_cross_attention_model(audio_feature_dim, video_feature_dim): # Renamed function
    """
    Create a non-branched hybrid model using Conv1D for audio and LSTM/BiLSTM for video,
    with added self-attention and cross-modal attention layers.
    ...
    """
    print("Creating non-branched hybrid model with Self & Cross-Attention:") # Updated description
    ...
    print(f"- Attention Key Dim: {ATTENTION_KEY_DIM}")
    print(f"- Merged Dense Units: {MERGED_DENSE_UNITS}")
    print(f"- Learning rate: {LEARNING_RATE} with warm-up and cosine decay")

    # --- Audio Branch (Conv1D + Self-Attention) ---
    audio_input = Input(shape=(None, audio_feature_dim), name='audio_input')
    audio_masked = Masking(mask_value=0.0)(audio_input)
    audio_x = Conv1D(AUDIO_CONV_FILTERS[0], kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT))(audio_masked)
    audio_x = BatchNormalization()(audio_x)
    audio_x = SpatialDropout1D(0.2)(audio_x)
    audio_x = MaxPooling1D(pool_size=2)(audio_x)
    audio_x = Conv1D(AUDIO_CONV_FILTERS[1], kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT))(audio_x)
    audio_x = BatchNormalization()(audio_x)
    audio_features_before_self_attention = SpatialDropout1D(0.2)(audio_x)
    # Audio Self-Attention
    audio_self_attention_output = MultiHeadAttention(num_heads=ATTENTION_HEADS, key_dim=ATTENTION_KEY_DIM, dropout=0.1, kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT))(query=audio_features_before_self_attention, value=audio_features_before_self_attention, key=audio_features_before_self_attention)
    audio_self_attended = LayerNormalization()(Add()([audio_features_before_self_attention, audio_self_attention_output])) # Output of audio self-attention

    # --- Video Branch (LSTM/BiLSTM + Self-Attention) ---
    ...
    video_features_before_self_attention = Bidirectional(LSTM(VIDEO_LSTM_UNITS[1], return_sequences=True, dropout=0.3, recurrent_dropout=0.2, kernel_regularizer=l2(L2_REGULARIZATION), recurrent_regularizer=l2(L2_REGULARIZATION/2), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT), recurrent_constraint=MaxNorm(MAX_NORM_CONSTRAINT)))(video_x)
    video_features_before_self_attention = Dropout(0.3)(video_features_before_self_attention)
    # Video Self-Attention
    video_self_attention_output = MultiHeadAttention(num_heads=ATTENTION_HEADS, key_dim=ATTENTION_KEY_DIM, dropout=0.1, kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT))(query=video_features_before_self_attention, value=video_features_before_self_attention, key=video_features_before_self_attention)
    video_self_attended = LayerNormalization()(Add()([video_features_before_self_attention, video_self_attention_output])) # Output of video self-attention

    # --- Cross-Modal Attention ---
    # Audio attends to Video
    audio_cross_attention_output = MultiHeadAttention(
    ...
    )(query=audio_self_attended, value=video_self_attended, key=video_self_attended)
    audio_refined = LayerNormalization()(Add()([audio_self_attended, audio_cross_attention_output]))

    # Video attends to Audio
    video_cross_attention_output = MultiHeadAttention(
        num_heads=ATTENTION_HEADS, key_dim=ATTENTION_KEY_DIM, dropout=0.1,
        kernel_regularizer=l2(L2_REGULARIZATION), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT),
        name='video_cross_attention'
    )(query=video_self_attended, value=audio_self_attended, key=audio_self_attended)
    video_refined = LayerNormalization()(Add()([video_self_attended, video_cross_attention_output]))

    # --- Pooling after Cross-Attention ---
    audio_pooled = GlobalMaxPooling1D()(audio_refined)
    video_pooled = GlobalMaxPooling1D()(video_refined)

    # --- Fusion ---
    merged = Concatenate()([audio_pooled, video_pooled])
...
def train_model():
    """Main function to train the LSTM/Conv1D model with self & cross-attention."""
    print("Starting LSTM/Conv1D + Self & Cross-Attention model training...") # Updated print
    ...
    train_video, video_mean, video_std = normalize_features(train_video) # Normalize video too
    val_video, _, _ = normalize_features(val_video, mean=video_mean, std=video_std)
    ...
    model = create_lstm_conv1d_cross_attention_model(audio_feature_dim, video_feature_dim) # Updated function call
    model.summary()

    # Define callbacks (updated checkpoint name)
    checkpoint_filename = 'best_model_lstm_conv1d_cross_attention.keras' # New checkpoint name

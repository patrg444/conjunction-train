...
enhanced with ONLY cross-modal attention between branches before fusion.
(Self-attention within branches is removed for ablation study).
"""
...
from tensorflow.keras.layers import Activation, Add, SpatialDropout1D, LayerNormalization, MultiHeadAttention
...
ATTENTION_HEADS = 4 # Number of attention heads (used for cross-attention)
ATTENTION_KEY_DIM = 64 # Key dimension for attention (used for cross-attention)

print("NON-BRANCHED HYBRID MODEL WITH CONV1D (AUDIO) + LSTM (VIDEO) + CROSS-ATTENTION ONLY") # Updated Title
...
def create_lstm_conv1d_cross_attention_only_model(audio_feature_dim, video_feature_dim): # Renamed function
    """
    Create a non-branched hybrid model using Conv1D for audio and LSTM/BiLSTM for video,
    with only cross-modal attention layers (no self-attention).
    ...
    """
    print("Creating non-branched hybrid model with Cross-Attention Only:") # Updated description
    ...
    audio_x = BatchNormalization()(audio_x)
    audio_features_processed = SpatialDropout1D(0.2)(audio_x) # Output before cross-attention
    ...
    video_x = Bidirectional(LSTM(VIDEO_LSTM_UNITS[1], return_sequences=True, dropout=0.3, recurrent_dropout=0.2, kernel_regularizer=l2(L2_REGULARIZATION), recurrent_regularizer=l2(L2_REGULARIZATION/2), kernel_constraint=MaxNorm(MAX_NORM_CONSTRAINT), recurrent_constraint=MaxNorm(MAX_NORM_CONSTRAINT)))(video_x)
    video_features_processed = Dropout(0.3)(video_x) # Output before cross-attention
    ...
    )(query=audio_features_processed, value=video_features_processed, key=video_features_processed) # Query: Audio, Key/Value: Video
    audio_refined = LayerNormalization()(Add()([audio_features_processed, audio_cross_attention_output]))
    ...
    )(query=video_features_processed, value=audio_features_processed, key=audio_features_processed) # Query: Video, Key/Value: Audio
    video_refined = LayerNormalization()(Add()([video_features_processed, video_cross_attention_output]))
...
def train_model():
    """Main function to train the LSTM/Conv1D model with cross-attention only."""
    print("Starting LSTM/Conv1D + Cross-Attention Only model training...") # Updated print
    ...
    model = create_lstm_conv1d_cross_attention_only_model(audio_feature_dim, video_feature_dim) # Updated function call
    model.summary()

    # Define callbacks (updated checkpoint name)
    checkpoint_filename = 'best_model_lstm_conv1d_cross_attention_only.keras' # New checkpoint name

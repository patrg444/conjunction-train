...
# Pass criteria and loss_weights as arguments
def validate(model, dataloader, criterion_humor, criterion_aux, loss_weights, device, use_video, use_text):
    model.eval()
    running_loss = 0.0
    # Initialize accumulators for metrics
    all_labels = {'emotion': [], 'humor': [], 'smile': [], 'laugh': [], 'joke': []}
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader:
            val_loss, val_acc, val_f1, val_metrics = validate(
                model, val_loader, criterion_humor, criterion_aux, loss_weights, device, use_video, use_text
            )

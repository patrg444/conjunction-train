...
Version 5: Maps 'calm' emotion to 'neutral' emotion with continuous indices
"""
...
def load_emotion_data(data_dir, test_size=0.1, use_cache=False, cache_dir="cache"):
    ...
    """
    cache_file = os.path.join(cache_dir, "wav2vec_data_cache_neutral_calm_v2.npz")
    ...
    # Map emotion labels to indices for consistent encoding (FIXED: continuous indices)
    # We're mapping 'calm' to 'neutral' (index 0) and reassigning all other indices to be consecutive
    # This gives us 6 emotion classes total
    emotion_to_index = {
    ...
    # Debug: Print the unique values in original and encoded labels
    print("\nUnique label values:")
    ...
    # Add a check to ensure no overlap between train and validation sets
    train_sample_ids = [hash(str(arr)) for arr in X_train[:5]]
    val_sample_ids = [hash(str(arr)) for arr in X_val[:5]]
    overlap_count = len(set(train_sample_ids).intersection(set(val_sample_ids)))
    
    if overlap_count > 0:
        print(f"WARNING: {overlap_count} samples overlap between training and validation sets!")
    else:
...
def train_model(data_dir, batch_size=64, epochs=100, learning_rate=0.001, dropout_rate=0.5, 
    ...
    if model_name is None:
        ...
        model_name = f"wav2vec_audio_only_six_classes_{timestamp}"

...
with reorganized data loading and fixes Keras/TF compatibility issues
Fixed v5: Corrects ResourceVariable access for TF compatibility
"""
...
class SelfAttentionLayer(Layer):
    def __init__(self, dim, num_heads=1, **kwargs): # Added num_heads parameter
        ...
        # Instantiate MultiHeadAttention layer in __init__
    ...
    def call(self, inputs):
        ...
        # Use the pre-instantiated MultiHeadAttention layer
        attention_output = self.mha(query=query, key=key, value=value)
    ...
    def get_config(self):
        ...
        # Add num_heads to config for saving/loading
...
def load_emotion_data(data_dir, test_size=0.1, use_cache=False, cache_dir="cache"):
    ...
    # Create train/val split - attempt stratified split first
    try:
        ...
        # If stratification fails (likely due to small sample size in local test)
        # fall back to a non-stratified split
        print(f"Warning: Stratified split failed ({e}). Falling back to non-stratified split.")
        X_train, X_val, y_train, y_val = train_test_split(
            X, y, test_size=test_size, random_state=SEED # No stratify=y
        )
    ...
    unique_train_classes, train_counts = np.unique(np.argmax(to_categorical(y_train, num_classes=num_classes), axis=1), return_counts=True)
    
    # Calculate weights only for classes present in the training set
    total_train_samples = len(y_train)
    for c, count in zip(unique_train_classes, train_counts):
        # Avoid division by zero if a class somehow has 0 samples despite being unique
        if count > 0: 
            # Standard class weight calculation: total_samples / (num_classes * count)
            # Using num_classes found in the *original* full dataset for weighting consistency
            # Corrected: Use num_classes instead of len(classes)
        ...
        print(f"  Class {c} ({emotion_name}): {count} samples (weight: {class_weights[c]:.4f})")

    # Print classes missing from the training set (relevant for small local tests)
    missing_classes = set(range(num_classes)) - set(unique_train_classes)
    if missing_classes:
        print(f"  Classes missing from training set (assigned weight 1.0): {sorted(list(missing_classes))}")
        
    print(f"\nTrain samples: {len(X_train)}")
    print(f"Validation samples: {len(X_val)}")
    
    # Convert labels to one-hot encoding
    # Convert labels to one-hot encoding using the full number of classes
...
# Custom learning rate scheduler with warm-up - FIXED for TF compatibility
class WarmUpReduceLROnPlateau(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if epoch < self.warmup_epochs:
            ...
            if self.verbose > 0:
                print(f'\nEpoch {epoch+1}: WarmUpReduceLROnPlateau setting learning rate to {float(learning_rate):.6f}.')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.monitor_op(current, self.best):
            ...
            if self.wait >= self.patience:
                ...
                # Using numpy() instead of value() for ResourceVariable access
                ...
                if old_lr > self.min_lr:
                    ...
                    if self.verbose > 0:
                        print(f'\nEpoch {epoch+1}: WarmUpReduceLROnPlateau reducing learning rate to {float(new_lr):.6f}.')
...
def build_model(input_shape, num_classes, dropout_rate=0.5):
    ...
    Fixed to use Keras-compatible operations instead of raw TF ops.
    """
    ...
    x = SelfAttentionLayer(dim=64, num_heads=1)(x)
    ...
    # Use Keras Softmax layer instead of tf.nn.softmax
...
def train_model(args):
    ...
    if model_name is None:
        ...
        model_name = f"wav2vec_audio_only_fixed_v5_{timestamp}"

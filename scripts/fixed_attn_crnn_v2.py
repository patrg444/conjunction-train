...
def load_wav2vec_data(data_dirs, length_percentile=99, recursive=True):
    ...
    # First pass - collect all files and their features/labels
    for data_dir in data_dirs:
        pattern = os.path.join(data_dir, '**', '*.npz') if recursive else os.path.join(data_dir, '*.npz')
        npz_files = glob.glob(pattern, recursive=recursive)
        ...
        for npz_file in npz_files:
            try:
                ...
                sequence_lengths.append(features.shape[0])  # Record sequence length
    ...
    # Analyze sequence lengths and apply cutoff
    ...
    for p, val in zip(percentiles, percentile_values):
        print(f"{p:3}th = {int(val):4}")

    cutoff_length = int(np.percentile(sequence_lengths, length_percentile))
    print(f"\nUsing cutoff at {length_percentile}th percentile: {cutoff_length} frames")

    # Filter out samples longer than the cutoff
    filtered_features = []
    filtered_labels = []
    filtered_files = []
    
    for features, label, file_path, seq_len in zip(all_features, all_labels, all_file_paths, sequence_lengths):
        if seq_len <= cutoff_length:
            filtered_features.append(features)
            filtered_labels.append(label)
            filtered_files.append(file_path)
    
    dropped_count = len(all_features) - len(filtered_features)
    if dropped_count > 0:
        drop_percent = (dropped_count / len(all_features)) * 100
        print(f"Dropped {dropped_count} / {len(all_features)} files ({drop_percent:.1f} %) longer than {cutoff_length} frames")
    
    # Convert features to numpy array and split into train/val
    X = filtered_features
    y = np.array(filtered_labels)

    # Print data stats
    print(f"\nFinal dataset: {len(X)} samples with {len(set(y))} emotion classes")
    ...
    # Pad sequences to cutoff length
    X_train_padded = np.zeros((len(X_train), cutoff_length, X_train[0].shape[1]))
    X_val_padded = np.zeros((len(X_val), cutoff_length, X_val[0].shape[1]))

    for i, x in enumerate(X_train):
        length = min(x.shape[0], cutoff_length)  # Ensure we don't exceed the cutoff
        X_train_padded[i, :length, :] = x[:length, :]

    for i, x in enumerate(X_val):
        length = min(x.shape[0], cutoff_length)  # Ensure we don't exceed the cutoff
        X_val_padded[i, :length, :] = x[:length, :]

    print(f"Padded/trimmed all sequences to {cutoff_length} frames")
...
def main():
    ...
    parser.add_argument('--length_percentile', type=int, default=99, 
                        help='Percentile for sequence length cutoff (default: 99, meaning drop longest 1%)')
    ...
    # Load data
    (X_train, y_train), (X_val, y_val), num_classes = load_wav2vec_data(
        args.data_dirs, 

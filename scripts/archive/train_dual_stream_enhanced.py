#!/usr/bin/env python3
"""
Enhanced training script for the dual-stream LSTM model for multimodal emotion recognition.
This implements the architecture with stronger regularization to combat overfitting:
- Added L2 regularization to all LSTM and Dense layers
- Increased dropout rates
- Enhanced training parameters (patience, learning rate scheduling)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("train_dual_stream_enhanced.log"),
...
def define_dual_stream_model(video_input_shape, audio_input_shape, num_classes):
    """Defines the dual-stream LSTM model architecture with stronger regularization.
    ...
    # Higher dropout rates for combating overfitting
    lstm_dropout = 0.4
    dense_dropout = 0.5
    ...
    # --- Video Stream ---
    # Using a stacked LSTM architecture for video with L2 regularization
    video_lstm1 = LSTM(64, return_sequences=True, 
                      ...
                      name='video_lstm1')(video_input)
    video_lstm1 = Dropout(lstm_dropout, name='video_dropout1')(video_lstm1)
    video_lstm2 = LSTM(32, return_sequences=False, 
                      kernel_regularizer=regularizers.l2(kernel_l2),
                      recurrent_regularizer=regularizers.l2(recurrent_l2),
                      bias_regularizer=regularizers.l2(bias_l2),
                      name='video_lstm2')(video_lstm1)
    video_lstm2 = Dropout(lstm_dropout, name='video_dropout2')(video_lstm2)

    # --- Audio Stream ---
    # Using a stacked LSTM architecture for audio with L2 regularization
    audio_lstm1 = LSTM(64, return_sequences=True, 
                      kernel_regularizer=regularizers.l2(kernel_l2),
                      recurrent_regularizer=regularizers.l2(recurrent_l2),
                      bias_regularizer=regularizers.l2(bias_l2),
                      name='audio_lstm1')(audio_input)
    audio_lstm1 = Dropout(lstm_dropout, name='audio_dropout1')(audio_lstm1)
    audio_lstm2 = LSTM(32, return_sequences=False, 
                      kernel_regularizer=regularizers.l2(kernel_l2),
                      recurrent_regularizer=regularizers.l2(recurrent_l2),
                      bias_regularizer=regularizers.l2(bias_l2),
                      name='audio_lstm2')(audio_lstm1)
    audio_lstm2 = Dropout(lstm_dropout, name='audio_dropout2')(audio_lstm2)
    ...
    dense1 = Dense(64, activation='relu', 
                  ...
                  bias_regularizer=regularizers.l2(bias_l2),
                  name='dense1')(fusion)
    dropout1 = Dropout(dense_dropout, name='dropout_dense1')(dense1)
    dense2 = Dense(32, activation='relu', 
                  kernel_regularizer=regularizers.l2(kernel_l2),
                  bias_regularizer=regularizers.l2(bias_l2),
                  name='dense2')(dropout1)
    dropout2 = Dropout(dense_dropout, name='dropout_dense2')(dense2)
...
def train_model(
    ...
    y_val,
    epochs=100,
    batch_size=16,  # Reduced from 32 to introduce more noise
    model_save_path='models/dual_stream_enhanced/'
):
    ...
    reduce_lr = ReduceLROnPlateau(
        monitor='val_loss',
        factor=0.7,  # More gradual decrease
...
def evaluate_model(model, X_test_video, X_test_audio, y_test, class_names, output_dir="model_evaluation/dual_stream_enhanced"):
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Train the enhanced dual-stream LSTM model for emotion recognition')
    ...
    parser.add_argument('--model_dir', type=str, default='models/dual_stream_enhanced/',
                        help='Directory to save model checkpoints (default: models/dual_stream_enhanced/)')
    parser.add_argument('--eval_dir', type=str, default='model_evaluation/dual_stream_enhanced/',
                        help='Directory to save evaluation results (default: model_evaluation/dual_stream_enhanced/)')
    ...
    final_model_path = os.path.join(MODEL_SAVE_PATH, 'final_model_enhanced.h5')

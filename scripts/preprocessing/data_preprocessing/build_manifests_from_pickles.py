...
def build_manifests_from_pickles():
    ...
    # --- Inspection of language_data ---
    print("\n--- Inspecting language_sdk.pkl data ---")
    print(f"Successfully loaded data. Type: {type(language_data)}")

    if isinstance(language_data, dict):
        print(f"Number of entries (keys): {len(language_data)}")
        print("Example keys:")
        # Print first 5 keys
        for i, key in enumerate(language_data.keys()):
            if i >= 5:
                break
            print(f"- {key}")
        ...
        for i, (key, value) in enumerate(language_data.items()):
            if i >= 3:
                break
            print(f"Key: {key}")
    ...
    for video_id in tqdm(train_ids):
        ...
        # Get transcript - only the punchline sentence
        lang_entry = language_data.get(video_id, {})
        transcript = lang_entry.get('punchline_sentence', '')

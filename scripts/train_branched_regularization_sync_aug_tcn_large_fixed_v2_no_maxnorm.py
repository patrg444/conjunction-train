...
1. Enhanced regularization - balanced L2, spatial dropout
...
VARIANT: Removed MaxNorm Constraints
"""
...
# from tensorflow.keras.constraints import MaxNorm # MaxNorm removed
import time
import glob
import random
...
# MAX_NORM_CONSTRAINT = 3.0  # MaxNorm removed
...
print("VARIANT: Removed MaxNorm Constraints")
...
def attention_block(x, num_heads=NUM_HEADS, key_dim=KEY_DIM, dropout=ATTENTION_DROPOUT, l2_reg=L2_REGULARIZATION):
    ...
    attention_output = MultiHeadAttention(
        ...
        # kernel_constraint=MaxNorm(max_value=MAX_NORM_CONSTRAINT) # Removed
    )(x, x)
...
def create_enhanced_large_model_with_regularization_and_tcn(audio_feature_dim, video_feature_dim):
    ...
    # print(f"- Weight constraint: {MAX_NORM_CONSTRAINT}") # Removed
    print("- Weight constraint: None")
    ...
    audio_x = Bidirectional(LSTM(
        ...
        # kernel_constraint=MaxNorm(max_value=MAX_NORM_CONSTRAINT), # Removed
        # recurrent_constraint=MaxNorm(max_value=MAX_NORM_CONSTRAINT) # Removed
    ))(audio_x)
...
def train_model():
    ...
    print(f"- Using L2 regularization: {L2_REGULARIZATION}")
    print("- MaxNorm constraint: REMOVED") # Added print for MaxNorm removal
    ...
    checkpoint = ModelCheckpoint(
        'best_model_tcn_large_fixed_v2_no_maxnorm.keras', # Adjusted checkpoint name
    ...
    model.load_weights('best_model_tcn_large_fixed_v2_no_maxnorm.keras') # Adjusted checkpoint name

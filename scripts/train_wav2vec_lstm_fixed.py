...
Modified to load *.npz wav2vec embeddings directly from models/wav2vec.
"""
...
def load_data_paths_and_labels(audio_base_dir, video_base_dir_ravdess, video_base_dir_cremad):
    """Finds corresponding audio embedding and video feature files and extracts labels."""
    # Update to look for audio embedding files (.npz) in models/wav2vec directory
    audio_files = glob.glob(os.path.join(audio_base_dir, "ravdess_*.npz")) + \
                  glob.glob(os.path.join(audio_base_dir, "cremad_*.npz"))
    ...
    for audio_file_path in tqdm(audio_files, desc="Matching files"):
        ...
        # Parse the filename based on its format
        if "ravdess_" in base_name.lower():  # RAVDESS 
            # Example: ravdess_03-01-06-01-02-01-12.npz
            # Extract the parts after ravdess_
            parts = base_name.replace("ravdess_", "").split('-')
            actor_id = parts[-1]  # Last part is the actor ID
            
            # Find the actor folder
            actor_folder = f"Actor_{actor_id}"
            potential_video_file = os.path.join(video_base_dir_ravdess, actor_folder, base_name.replace("ravdess_", "") + ".npz")
            ...
            if os.path.exists(potential_video_file):
                ...
                try:
                    ...
                    print(f"Error parsing RAVDESS file {base_name}: {e}")
            else: 
                skipped_video += 1
        
        elif "cremad_" in base_name.lower():  # CREMA-D
            # Example: cremad_1001_DFA_ANG_XX.npz
            # Extract the parts after cremad_
            parts = base_name.replace("cremad_", "").split('_')
            
            # Find the corresponding video file
            potential_video_file = os.path.join(video_base_dir_cremad, base_name.replace("cremad_", "") + ".npz")
            
            if os.path.exists(potential_video_file):
                video_file = potential_video_file
                try:
                    emotion_code = parts[2]  # In CREMA-D, emotion is the 3rd part
                    if emotion_code in emotion_map:
                        label = np.zeros(len(emotion_map))
                        label[emotion_map[emotion_code]] = 1
                except Exception as e:
                    print(f"Error parsing CREMA-D file {base_name}: {e}")
            else:
                skipped_video += 1
        else:
            print(f"Unknown file format: {base_name}")
...
# Modify the Wav2VecSyncGenerator to load NPZ files with wav2vec_features key
class ModifiedWav2VecSyncGenerator(Wav2VecSyncGenerator):
    def __load_audio_embedding(self, path):
        try:
            # Load NPZ file and extract the wav2vec_features
            with np.load(path) as data:
                if 'wav2vec_features' in data:
                    return data['wav2vec_features']
                else:
                    print(f"Error: 'wav2vec_features' key not found in {path}")
                    return None
        except Exception as e:
...
def train_model():
    ...
    # UPDATED: Use models/wav2vec for audio embeddings
    AUDIO_DIR = "models/wav2vec"  # Updated to use the new location
    RAVDESS_VIDEO_FEAT_DIR = "data/ravdess_features_facenet"
    CREMA_D_VIDEO_FEAT_DIR = "data/crema_d_features_facenet"

    # Load file paths and labels
    try:
        # Update function call to use new audio dir
        video_files, audio_files, all_labels = load_data_paths_and_labels(
            AUDIO_DIR, RAVDESS_VIDEO_FEAT_DIR, CREMA_D_VIDEO_FEAT_DIR
        )
    ...
    try:
        ...
        with np.load(train_audio_files[0]) as data:
            if 'wav2vec_features' in data:
                temp_audio_embedding = data['wav2vec_features']
                audio_embedding_dim = temp_audio_embedding.shape[1]
                audio_input_shape = (None, audio_embedding_dim) # Time steps is variable
            else:
                raise KeyError("'wav2vec_features' key not found in NPZ file")
    except Exception as e:
        print(f"Error loading sample Wav2Vec2 embedding to determine shape: {e}")
        sys.exit(1)

    print("\nCreating data generators...")
    # Use our modified generator that handles NPZ files
    train_generator = ModifiedWav2VecSyncGenerator(
        train_video_files, train_audio_files, train_labels, batch_size=BATCH_SIZE, shuffle=True
    )
    val_generator = ModifiedWav2VecSyncGenerator(

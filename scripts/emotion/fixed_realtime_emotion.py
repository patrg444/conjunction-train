#!/usr/bin/env python3
"""
Fixed version of the real-time emotion recognition system
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("fixed_realtime_emotion.log"),
...
class EmotionRecognizer:
    ...
    def __init__(self, model_path, opensmile_path, config_file,
        ...
        # Try to load the model with improved compatibility
        try:
            # Custom LSTM compatibility layer
            class FixedLSTMLayer(tf.keras.layers.LSTM):
                def __init__(self, *args, **kwargs):
                    # Remove all problematic parameters
                    for param in ['time_major', 'dropout_state_filter_visitor']:
                        if param in kwargs:
                            if self.debug:
                                logging.debug(f"Removing {param}={kwargs[param]} parameter from LSTM")
                            kwargs.pop(param, None)
                    super(FixedLSTMLayer, self).__init__(*args, **kwargs)
            
            # Try the flexible approach
            try:
                # Register our custom layer to be used for all LSTM layers
                original_lstm = tf.keras.layers.LSTM
                tf.keras.layers.LSTM = FixedLSTMLayer
                
                logging.info("Using global LSTM layer override")
                self.model = tf.keras.models.load_model(model_path, compile=False)
                
                # Restore original class
                tf.keras.layers.LSTM = original_lstm
                
                logging.info("Model loaded successfully with LSTM patching")
                
            except Exception as e:
                logging.warning(f"Global LSTM override failed: {str(e)}")
                
                # Try with custom objects approach
                self.model = tf.keras.models.load_model(
                    model_path,
                    custom_objects={'LSTM': FixedLSTMLayer},
                    compile=False
                )
            ...
            logging.info(f"Model input shapes: {[input.shape for input in self.model.inputs]}")
            logging.info(f"Model output shapes: {[output.shape for output in self.model.outputs]}")
            
        except Exception as e:
            self.model = None
            logging.error(f"Error loading model: {str(e)}")
            logging.error("Will run in face detection only mode")
            import traceback
            traceback.print_exc()
    ...
    def start(self):
        ...
        try:
            while self.running:
                ...
                # If we have both types of features and a model, predict emotion
                if face_features is not None and audio_features is not None and self.model is not None:
                    ...
                    # Make prediction
                    try:
                        # Different models might have different input schemes
                        if len(self.model.inputs) == 2:
                            # Check for time dimension requirement
                            if len(self.model.inputs[0].shape) > 2:  # if model expects time dimension
                                if self.debug:
                            ...
                            # Typical case: separate inputs for audio and video
                            prediction = self.model.predict([face_features_batch, audio_features_batch], verbose=0)
                            if self.debug:
                                logging.info(f"Made prediction with two inputs: {type(prediction)}")
                        else:
                            # Single input model (less common)
                            combined_features = np.concatenate([face_features_batch, audio_features_batch], axis=1)
                            prediction = self.model.predict(combined_features, verbose=0)
                            if self.debug:
                                logging.info(f"Made prediction with combined inputs: {type(prediction)}")
                        
                        # Get probabilities (handle different output shapes)
                        if isinstance(prediction, list):
                            # Multiple outputs, take the first one
                            probs = prediction[0][0]
                            if self.debug:
                                logging.info(f"Multiple outputs: {len(prediction)}, using first")
                        else:
                            # Single output
                            probs = prediction[0]
                            if self.debug and self.frame_count % 10 == 0:
                                logging.info(f"Prediction: {probs}")
                        
                        # Limit to our emotion labels
                        if len(probs) > len(EMOTIONS):
                            probs = probs[:len(EMOTIONS)]
                        
                        # Update smoothing buffers
                        for i, prob in enumerate(probs):
                            if i < len(self.emotion_buffers):
                        ...
                        # Create random probabilities for demonstration
                    ...
                    # Create dummy probabilities (equal for all emotions)
    ...
    def _create_display(self, frame, face_features, audio_features,
        ...
        # Add feature status
        face_status = "DETECTED" if np.any(face_features != 0) else "NOT DETECTED"
        face_color = (0, 255, 0) if np.any(face_features != 0) else (0, 0, 255)

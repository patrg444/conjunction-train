...
print("VIDEO ONLY FACENET LSTM") # Updated identifier
    ...
    for facenet_file in tqdm(facenet_files, desc="Extracting labels"):
        ...
        if label is not None:
            ...
            if os.path.exists(facenet_file) and os.path.getsize(facenet_file) > 0:
                 # Optional: Add a check to load the npz and see if 'features' key exists
                 try:
                     with np.load(facenet_file) as data:
                         if 'features' in data and data['features'].shape[0] > 0:
                             valid_facenet_files.append(facenet_file)
                             labels.append(label)
                         else:
                             print(f"Warning: Skipping {facenet_file} - 'features' key missing or empty.")
                             skipped_label += 1
...
def train_model():
    ...
    # Define feature directories (POINT TO FACENET FEATURES)
    RAVDESS_FACENET_DIR = "data/ravdess_features_facenet" # Use Facenet features
    CREMA_D_FACENET_DIR = "data/crema_d_features_facenet" # Use Facenet features
    ...
    for f_path in tqdm(train_facenet_files_final, desc="Checking lengths"):
        try:
            # Load the npz file and get the shape of the 'features' array
            with np.load(f_path) as data:
                 if 'features' in data:
                     ...
                     print(f"Warning: 'features' key not found in {f_path}")
        except Exception as e:

...
# coding: utf-8 -*-
"""
Smooth Camera Demo for Emotion Recognition
...
Now with smoother emotion visualization using a moving average.
"""
...
parser = argparse.ArgumentParser(description='Smooth Camera Demo')
...
parser.add_argument('--window_size', type=int, default=15,
                    help='Size of the smoothing window (default: 15 frames)')
...
for buffer in emotion_buffers:
    # Start with some random initial values
    for _ in range(smoothing_window_size):
        buffer.append(np.random.random() * 0.5)  # Start with low random values

def main():
    """Main function to run the camera demo."""
    global running, last_frame_time, emotion_buffers
    ...
    try:
        while running:
            ...
            # Update emotion probabilities - now with a bit of continuity from previous values
            for i, buffer in enumerate(emotion_buffers):
                # Generate a new random value - with some influence from the previous value
                prev_avg = sum(buffer) / len(buffer)
                # Random walk - new value is based on previous plus some random change
                # This makes the random values move more smoothly even before averaging
                new_value = max(0, min(1, prev_avg + (np.random.random() - 0.5) * 0.15))
                buffer.append(new_value)

            # Calculate smoothed emotion probabilities
            smoothed_probs = [sum(buffer) / len(buffer) for buffer in emotion_buffers]
            
            # Find the dominant emotion based on smoothed probabilities
            ...
            cv2.putText(display, f"Emotion: {dominant_emotion}", (20, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, 
                        emotion_colors[dominant_emotion_idx], 2)
            
            # Add confidence display
            confidence = smoothed_probs[dominant_emotion_idx]
            cv2.putText(display, f"Confidence: {confidence:.2f}", (20, 80), 
            ...
            # Bar visualization for emotions
            bar_height = 20
            bar_width = 200
            x_start = 20
            y_start = 120
            ...
            for i, emotion in enumerate(emotions):
                ...
                # Draw filled portion using smoothed value
            ...
            # Show frame
            cv2.imshow('Smooth Camera Demo (Moving Average Emotions)', display)

...
# Import the specialized video-only generator
try:
...
def train_model():
    ...
    # Create data generators with specialized VideoOnlyFacenetGenerator
    print("\nCreating data generators (Precomputed Facenet - Video Only)...")
    # Use the specialized generator that handles video-only mode properly
    train_generator = VideoOnlyFacenetGenerator(
        video_feature_files=train_facenet_files_final,
        labels=train_labels_final,
        batch_size=BATCH_SIZE,
        shuffle=True,
        max_seq_len=max_seq_len, # Use calculated max_seq_len
        audio_mode='video_only'  # For compatibility with existing code
    )
    val_generator = VideoOnlyFacenetGenerator(
        video_feature_files=val_facenet_files_final,

#!/usr/bin/env python3
"""
Real-time Emotion Recognition using the same processing pipeline as our trained model.
Processes webcam video at 15 FPS and microphone audio using OpenSMILE and FaceNet.
Uses the branched model trained on RAVDESS and CREMA-D datasets.
"""
...
class AudioProcessor:
    ...
    def __init__(self, opensmile_path, config_file, temp_dir="temp_extracted_audio"):
        ...
        logging.info(f"Audio processor initialized with OpenSMILE at: {opensmile_path}")
    ...
    def _record_and_process(self):
        ...
        # Process audio in 1-second chunks (chosen to balance latency and feature quality)
...
class EmotionRecognizer:
    ...
    def __init__(self, model_path, opensmile_path, config_file, 
        ...
        try:
            # Custom LSTM object to handle older versions
            ...
            # Load the model with custom objects to handle compatibility
        ...
        try:
            while self.running:
                ...
                if face_features is not None and audio_features is not None and self.model is not None:
                    ...
                    face_features = np.expand_dims(face_features, axis=0)  # Add batch dimension
                    ...
                    try:
                        ...
                        if len(self.model.inputs) == 2:
                            ...
                            prediction = self.model.predict([face_features, audio_features], verbose=0)
                        else:
                            # Single input model (less common)
                            combined_features = np.concatenate([face_features, audio_features], axis=1)

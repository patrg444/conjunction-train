#!/bin/bash
# Final fix that uses a working method to update learning rate

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77"

echo "===== Creating Final Working Fix ====="

# Upload a fixed script to the server
ssh -i $KEY_PATH $SERVER "cat > /home/ubuntu/audio_emotion/final_working_fix.py" << 'EOF'
#!/usr/bin/env python3
"""
WAV2VEC audio emotion recognition model with fixed learning rate update method.
"""
...
class WarmUpReduceLROnPlateau(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if epoch < self.warmup_epochs:
            ...
            # FIXED: Using assign method which works reliably
...
ssh -i $KEY_PATH $SERVER "chmod +x /home/ubuntu/audio_emotion/final_working_fix.py"

# Stop any existing processes
echo "Stopping any existing training processes..."
ssh -i $KEY_PATH $SERVER "pkill -f 'python.*wav2vec.*\.py' || true"

# Start training with the final fixed script
echo "Starting training with the correct fix..."
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="/home/ubuntu/audio_emotion/wav2vec_final_fix_${TIMESTAMP}.log"
ssh -i $KEY_PATH $SERVER "cd /home/ubuntu/audio_emotion && nohup python final_working_fix.py > $LOG_FILE 2>&1 &"
...
SSH_CMD="ls -t /home/ubuntu/audio_emotion/wav2vec_final_fix_*.log | head -1"
LATEST_LOG=\$(ssh -i \$KEY_PATH \$SERVER "\$SSH_CMD")
echo "Using log file: \$LATEST_LOG"
echo "==============================================================="

echo "Checking if training process is running..."
PROCESS_COUNT=\$(ssh -i \$KEY_PATH \$SERVER "ps aux | grep 'python.*final_working_fix.py' | grep -v grep | wc -l")
...
#!/bin/bash
# Script to download the model trained with the final working fix
...
echo "===== Final Working Fix Applied and Training Restarted ====="
echo "A completely new script with a working learning rate method has been deployed to the server."

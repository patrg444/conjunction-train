#!/usr/bin/env python3
"""
Extract audio from MP4 files, then process with wav2vec 2.0 and fuse with video model.

This script handles:
1. Extracting audio from .mp4 files in the RAVDESS and CREMA-D datasets
2. Extracting wav2vec features from the audio
3. Setting up a fusion with SlowFast video model
"""
...
def extract_audio_from_video(video_path, output_dir=None):
    ...
    try:
        # If output_dir is not specified, use temp directory
...
def process_ravdess_file(args):
    """Process a single RAVDESS video file."""
    video_path, output_dir, processor, model, device, temp_audio_dir = args
    ...
    try:
        ...
        audio_path = extract_audio_from_video(video_path, temp_audio_dir)
        ...
        # Remove temporary audio file if it was created in the temp directory
        if temp_audio_dir is None and os.path.exists(audio_path):
...
def process_cremad_file(args):
    """Process a single CREMA-D video file."""
    video_path, output_dir, processor, model, device, temp_audio_dir = args
    
    try:
        # Extract emotion from filename
        # Typical CREMA-D format: 1076_MTI_SAD_XX.mp4
        ...
        # Save features to NPZ file
        output_file = os.path.join(output_dir, f"cremad_{filename.replace('.mp4', '')}.npz")
        np.savez(
            output_file, 
            wav2vec_features=features, 
            label=label, 
...
def main():
    parser = argparse.ArgumentParser(description="Extract audio, wav2vec features and set up fusion model")
    ...
    parser.add_argument("--temp_audio_dir", type=str, default=None,
    ...
    if not ravdess_files:
        ...
        for file in tqdm(ravdess_files, desc="Processing RAVDESS"):
            result = process_ravdess_file((file, args.output_dir, processor, model, device, args.temp_audio_dir))
            if result:
    ...
    cremad_files = glob.glob(os.path.join(args.cremad_dir, "**/*.mp4"), recursive=True)
    
    if not cremad_files:
        print(f"No MP4 files found in {args.cremad_dir}")
    else:
        ...
        for file in tqdm(cremad_files, desc="Processing CREMA-D"):
            result = process_cremad_file((file, args.output_dir, processor, model, device, args.temp_audio_dir))
            if result:

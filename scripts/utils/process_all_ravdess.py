#!/usr/bin/env python3
"""
Script to process all downsampled RAVDESS files for feature extraction.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("process_all_ravdess.log"),
...
def process_all_files(input_dir, output_dir, model_name="VGG-Face", detector_backend="mtcnn", limit=None, num_workers=4, skip_existing=True):
    ...
    # Get list of video files with .mp4 extension (RAVDESS uses mp4 format)
    video_files = []
    # RAVDESS has a nested directory structure with Actor folders
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process all downsampled RAVDESS files for feature extraction")
    parser.add_argument("--input-dir", default="downsampled_videos/RAVDESS", 
                      help="Directory containing downsampled RAVDESS videos")
    parser.add_argument("--output-dir", default="ravdess_features", 

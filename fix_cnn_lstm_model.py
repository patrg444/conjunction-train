#!/usr/bin/env python3
"""
Simplified CNN-LSTM model based on diagnostic results.
Key changes:
1. Simpler architecture with fewer layers
2. Decreased regularization
3. Improved batch normalization
4. Fixed masking and padding handling
5. Modified learning rate scheduling
"""
...
# Reduced regularization as our diagnostic showed we can easily overfit the data
L2_REGULARIZATION = 0.0001  # Reduced significantly as our overfitting test was successful
DROPOUT_RATE = 0.3  # Reduced to avoid underfitting
...
print("IMPROVED CNN-LSTM MODEL WITH SIMPLIFIED ARCHITECTURE")
...
def create_model_architecture(input_dim):
    """Create a simplified model architecture based on diagnostic results."""
    print(f"Creating improved CNN-LSTM model with modified architecture:")
    ...
    # Use 0 as the mask value since zero-padding is common
    x = Masking(mask_value=0.0)(inputs)
    
    # Batch normalization helps stabilize training
    x = BatchNormalization()(x)
    
    # Simple bidirectional LSTM layer
    # Using minimal regularization since our diagnostic showed we can easily overfit
    lstm_layer = Bidirectional(LSTM(
        64,  # Reduced units
        return_sequences=False,  # Just get final summary
        dropout=DROPOUT_RATE, 
        recurrent_dropout=DROPOUT_RATE/2,  # Less aggressive on recurrent
    ))(x)
    
    # Layer normalization helps with training stability
    x = LayerNormalization()(lstm_layer)
...
def create_alternate_model(input_dim):
    """Create an alternate model that focuses on global features first."""
    print(f"Creating alternate model focusing on global feature aggregation:")
    print(f"- Input dimension: {input_dim}")
...
def train_model():
    ...
    print("Starting improved CNN-LSTM model training...\n")
    ...
    class_indices = [np.where(np.argmax(all_labels, axis=1) == i)[0] for i in range(NUM_CLASSES)]
    train_idx, val_idx = [], []
    ...
    lstm_model = create_model_architecture(input_dim)
    lstm_model.summary()
    ...
    models_dir = f"models/cnn_lstm_fixed_{timestamp}"
    ...
    alt_callbacks = [
        ModelCheckpoint(
            os.path.join(models_dir, 'alternate_model.h5'),
            monitor='val_accuracy',
            save_best_only=True,
            mode='max',
            verbose=1
    ...
    print(f"LSTM Model Validation Accuracy: {lstm_val_acc:.4f}")
    print(f"Alternate Model Validation Accuracy: {alt_val_acc:.4f}")

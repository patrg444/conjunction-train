#!/bin/bash
# Deploy the fixed Wav2Vec training script (fixed_v6_script_final.py) with the missing comma issue fixed
# This is a direct redeployment of the fixed original script, not a new version
...
LOG_FILE="wav2vec_fixed_comma_$TIMESTAMP.log"
...
cat > monitor_fixed_comma.sh << EOL
#!/bin/bash
...
echo "Finding the most recent fixed comma training log file..."
LOG_FILE=\$(ssh -i \$KEY_PATH \$EC2_HOST "find \$REMOTE_DIR -name 'wav2vec_fixed_comma_*.log' -type f -printf '%T@ %p\n' | sort -n | tail -1 | cut -f2- -d' '")
...
chmod +x monitor_fixed_comma.sh
echo "Created monitoring script: monitor_fixed_comma.sh"

# Create download script for the final model
cat > download_fixed_model.sh << EOL
#!/bin/bash
...
TIMESTAMP=\$(date +%Y%m%d_%H%M%S)
LOCAL_MODEL="wav2vec_fixed_model_\$TIMESTAMP.h5"

echo "Downloading fixed wav2vec model..."
...
echo "Created download script: download_fixed_model.sh"

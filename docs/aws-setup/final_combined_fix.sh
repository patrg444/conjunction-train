#!/bin/bash
# Final script to fix the array comparison issue when combining datasets

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Creating the final combined datasets script with fixed array comparison..."
...
# Create a training script that correctly combines both datasets
cat > scripts/train_combined_final.py << 'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Final fixed training script for both CREMA-D and RAVDESS datasets.
Fixed array comparison issue for combining datasets.
"""
...
print("FINAL COMBINED DATASETS TRAINING SCRIPT STARTING")
...
def train_model():
    ...
    # Combine available datasets - FIXED the array comparison logic
    all_audio = []
    all_video = []
    all_labels = None  # Initialize as None instead of []
    ...
    if cremad_audio is not None:
        ...
        if all_labels is None:  # Check if None instead of comparing with []
...
chmod +x scripts/train_combined_final.py

# Clear the current training log
> training.log

# Run the fixed training script using the proper conda environment
echo "Starting final combined datasets training script..." > training.log
source $HOME/anaconda3/etc/profile.d/conda.sh
conda activate tensorflow2_p310
echo "Conda environment activated, running script..." >> training.log
cd ~/emotion_training
nohup python scripts/train_combined_final.py >> training.log 2>&1 &
echo "Training restarted with fixed combined datasets script. Check training.log for progress."
ps aux | grep python >> training.log
EOF

echo "Final combined datasets script applied."

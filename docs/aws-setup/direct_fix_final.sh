#!/bin/bash
# Final attempt to fix and run the training script on AWS

INSTANCE_IP="98.82.121.48"
USERNAME="ec2-user"
KEY_FILE="emotion-recognition-key-20250322082227.pem"

echo "Creating a fixed script with direct conda execution..."
ssh -i "${KEY_FILE}" -o StrictHostKeyChecking=no ${USERNAME}@${INSTANCE_IP} << 'EOF'
cd ~/emotion_training

# Kill any previous training processes that might be stuck
pkill -f "python"

# Create a script that will run the fixed training
cat > run_fixed.sh << 'SCRIPTEOF'
#!/bin/bash
# Script to run training in the TensorFlow conda environment

# Setup conda
source $HOME/anaconda3/etc/profile.d/conda.sh
conda activate tensorflow2_p310

# Check if environment activated properly
if [[ $? -ne 0 ]]; then
  echo "ERROR: Failed to activate conda environment" >> run_errors.log
  exit 1
fi

echo "==== Environment Information ====" > training.log
...
# Create fixed training script with proper syntax correction for merged output
cat > scripts/train_fixed.py << 'PYEOF'
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
FIXED Training script for the branched model with proper encoding declaration.
Bug fixed: Corrected 'output = Dense...(output)' to 'output = Dense...(merged)'
"""
...
def create_model(audio_input_shape, video_input_shape):
    ...
    # Output layer - BUG FIXED: was using 'output' instead of 'merged'
...
chmod +x scripts/train_fixed.py
...
nohup python scripts/train_fixed.py >> training.log 2>&1 &
echo "Training restarted with fixed script. Check training.log for progress."
SCRIPTEOF

chmod +x run_fixed.sh
./run_fixed.sh

echo "Fixed script executed. Training should be running in the background."
echo "You can check training progress with: cat training.log"
EOF

echo "Final fix applied. Training should be running on the AWS instance."
echo "Wait a minute and then check training progress with:"
echo "cd aws-setup && ./direct_check_progress.sh"

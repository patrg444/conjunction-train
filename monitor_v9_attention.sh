#!/bin/bash
# Script to monitor the WAV2VEC Attention model (v9) training progress

KEY_PATH="~/Downloads/gpu-key.pem"
SERVER="ubuntu@54.162.134.77" 

echo "Finding the most recent v9 training log file..."
SSH_CMD="ls -t /home/ubuntu/audio_emotion/wav2vec_v9_attention_*.log | head -1"
LATEST_LOG=$(ssh -i $KEY_PATH $SERVER "$SSH_CMD")
echo "Using log file: $LATEST_LOG"
echo "==============================================================="

echo "Checking if training process is running..."
PROCESS_COUNT=$(ssh -i $KEY_PATH $SERVER "ps aux | grep 'python.*fixed_v9_attention.py' | grep -v grep | wc -l")

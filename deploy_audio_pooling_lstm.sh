...
LOCAL_SCRIPT_PATH="scripts/train_audio_pooling_lstm.py" # Path to the NEW training script
REMOTE_SCRIPT_NAME="train_audio_pooling_lstm.py" # Name for the script on the EC2 instance
REMOTE_DIR="/home/ec2-user/emotion_training" # Directory on EC2 for training files
LOG_PREFIX="training_audio_pooling_lstm" # Prefix for log files
PID_PREFIX="audio_pooling_lstm" # Prefix for PID files
# --- End Configuration ---
...
echo -e "${BLUE}===================================================================${NC}"
echo -e "${GREEN}    DEPLOYING AUDIO POOLING LSTM MODEL SCRIPT    ${NC}"
echo -e "${BLUE}===================================================================${NC}"
...
echo -e "${YELLOW}Transferring Audio Pooling LSTM script ($LOCAL_SCRIPT_PATH) to AWS instance...${NC}"
...
if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Generator script transfer failed.${NC}"
    exit 1
fi
...
echo \"Starting Audio Pooling LSTM model training...\" > $REMOTE_LOG_FILE
...
# Removed problematic TensorFlow version check line to avoid quoting issues

"
REMOTE_LAUNCH_SCRIPT="launch_audio_pooling_lstm.sh"
...
# Simplify escaping for printf + ssh cat > method
...
echo -e \"\${BLUE}==================================================================\${NC}\"
echo -e \"\${GREEN}    MONITORING AUDIO POOLING LSTM MODEL TRAINING (${TIMESTAMP})    \${NC}\"
echo -e \"\${BLUE}==================================================================\${NC}\"

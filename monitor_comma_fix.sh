#!/bin/bash
# Monitor training for the comma-fixed wav2vec model

# Variables
EC2_HOST="ubuntu@54.162.134.77"
KEY_PATH="~/Downloads/gpu-key.pem"
LOG_FILE="wav2vec_final_fix_patched_20250422_143530.log"
REMOTE_DIR="/home/ubuntu/audio_emotion"

echo "Finding the most recent patched training log file..."
LOG_FILE=$(ssh -i $KEY_PATH $EC2_HOST "find $REMOTE_DIR -name 'wav2vec_final_fix_patched_*.log' -type f -printf '%T@ %p\n' | sort -n | tail -1 | cut -f2- -d' '")

...
LOCAL_SCRIPT_PATH="scripts/train_audio_pooling_transformer_encoder.py" # Path to the NEW training script
REMOTE_SCRIPT_NAME="train_audio_pooling_transformer_encoder.py" # Name for the script on the EC2 instance
REMOTE_DIR="/home/ec2-user/emotion_training" # Directory on EC2 for training files
LOG_PREFIX="training_audio_pooling_transformer_encoder" # Prefix for log files
PID_PREFIX="audio_pooling_transformer_encoder" # Prefix for PID files
# --- End Configuration ---
...
echo -e "${BLUE}===================================================================${NC}"
echo -e "${GREEN}    DEPLOYING AUDIO POOLING TRANSFORMER ENCODER MODEL SCRIPT    ${NC}" # Updated Title
echo -e "${BLUE}===================================================================${NC}"
...
echo -e "${YELLOW}Transferring Audio Pooling Transformer Encoder script ($LOCAL_SCRIPT_PATH) to AWS instance...${NC}" # Updated description
...
# Transfer the generator script using SCP (Still needed for this model)
LOCAL_GENERATOR_PATH="scripts/audio_pooling_generator.py"
REMOTE_GENERATOR_NAME="audio_pooling_generator.py"
echo -e "${YELLOW}Transferring Audio Pooling Generator script ($LOCAL_GENERATOR_PATH) to AWS instance...${NC}"
scp -i "$KEY_FILE" "$LOCAL_GENERATOR_PATH" "$USERNAME@$INSTANCE_IP:$REMOTE_DIR/$REMOTE_GENERATOR_NAME"
if [ $? -ne 0 ]; then echo -e "${RED}Error: Generator script transfer failed.${NC}"; exit 1; fi
echo -e "${GREEN}Generator script transferred successfully!${NC}"
...
echo \"Starting Audio Pooling Transformer Encoder model training...\" > $REMOTE_LOG_FILE # Updated description
...
# Monitoring script for Audio Pooling Transformer Encoder model training # Updated description
...
echo -e \"\${BLUE}==================================================================\${NC}\"
echo -e \"\${GREEN}    MONITORING AUDIO POOLING TRANSFORMER ENCODER MODEL TRAINING (${TIMESTAMP})    \${NC}\" # Updated Title
echo -e \"\${BLUE}==================================================================\${NC}\"

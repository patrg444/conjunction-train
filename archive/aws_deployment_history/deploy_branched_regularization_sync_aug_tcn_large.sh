...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "cat > ${REMOTE_DIR}/run_branched_regularization_sync_aug_tcn_large.sh << 'EOL'
#!/bin/bash
cd ~/emotion_training

# Run the combined script with Python 3.9
nohup /usr/bin/python3.9 scripts/${SCRIPT_NAME} > ${LOG_FILE} 2>&1 &
echo \$! > branched_regularization_sync_aug_tcn_large_pid.txt
echo \"Training process started with PID \$(cat branched_regularization_sync_aug_tcn_large_pid.txt)\"
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "chmod +x ${REMOTE_DIR}/run_branched_regularization_sync_aug_tcn_large.sh"

# Run the script
echo "Starting training process on remote server..."
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "${REMOTE_DIR}/run_branched_regularization_sync_aug_tcn_large.sh"

#!/bin/bash
# Script to deploy the LSTM/Conv1D non-branched model to AWS
...
LOCAL_SCRIPT_PATH="scripts/train_lstm_conv1d_nonbranched.py" # Path to the new script
REMOTE_SCRIPT_PATH="${REMOTE_DIR}/scripts/train_lstm_conv1d_nonbranched.py" # Remote path for the new script
REMOTE_LAUNCH_SCRIPT_PATH="${REMOTE_DIR}/launch_lstm_conv1d.sh" # New launch script name

echo -e "${BLUE}===================================================================${NC}"
echo -e "${GREEN}    DEPLOYING LSTM/Conv1D NON-BRANCHED MODEL SCRIPT    ${NC}"
echo -e "${BLUE}===================================================================${NC}"
echo ""
...
echo -e "${YELLOW}Transferring LSTM/Conv1D script ($LOCAL_SCRIPT_PATH) to AWS instance...${NC}"
...
SCRIPT_PATH="scripts/train_lstm_conv1d_nonbranched.py" # Use the new script
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="training_lstm_conv1d_${TIMESTAMP}.log" # New log file pattern
PID_FILE="lstm_conv1d_${TIMESTAMP}_pid.txt" # New PID file pattern

echo "Starting LSTM/Conv1D non-branched model training..."
...
echo -e "${YELLOW}The specific log and PID files will have a timestamp like YYYYMMDD_HHMMSS and be named like 'training_lstm_conv1d_...' and 'lstm_conv1d_...'.${NC}"

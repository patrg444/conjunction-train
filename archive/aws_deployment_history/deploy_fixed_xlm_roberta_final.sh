...
echo -e "${BLUE}Deploying final fixed XLM-RoBERTa training script to EC2 instance...${NC}"

# Upload the fixed Python script to EC2
echo -e "${BLUE}Uploading fixed script to EC2...${NC}"
scp -i "$SSH_KEY" fixed_train_xlm_roberta_script_final.py ubuntu@$EC2_IP:/home/ubuntu/train_xlm_roberta_large_extended.py
...
# Start the training with the fixed script - using devices instead of gpus for updated API
echo -e "${BLUE}Starting training with fixed script...${NC}"
ssh -i "$SSH_KEY" ubuntu@$EC2_IP "cd /home/ubuntu && nohup python train_xlm_roberta_large_extended.py --train_manifest /home/ubuntu/ur_funny_train_humor_cleaned.csv --val_manifest /home/ubuntu/ur_funny_val_humor_cleaned.csv --model_name xlm-roberta-large --max_length 128 --batch_size 8 --learning_rate 1e-5 --epochs 15 --num_workers 4 --weight_decay 0.01 --dropout 0.1 --scheduler cosine --grad_clip 1.0 --log_dir training_logs_humor --exp_name xlm-roberta-large_extended_training --devices 1 --fp16 > training_output.log 2>&1 &"
...
if [ -z "$RUNNING_PROCESS" ]; then
    ...
    ssh -i "$SSH_KEY" ubuntu@$EC2_IP "cat training_output.log"
    exit 1
else
    ...
    echo -e "  ssh -i $SSH_KEY ubuntu@$EC2_IP 'tail -f /home/ubuntu/training_logs_humor/xlm-roberta-large_extended_training/training.log'"
fi

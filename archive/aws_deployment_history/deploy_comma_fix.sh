#!/bin/bash
# Script to deploy the fixed script with commas to the AWS server and restart training
...
LOCAL_FIXED_SCRIPT="fixed_script_local.py"

echo "===== Deploying Wav2Vec Learning Rate Comma Fix ====="

# 1. Back up the original script on the server
echo "Creating backup of the original script on the server..."
ssh -i $KEY_PATH $SERVER "cp $REMOTE_SCRIPT_PATH ${REMOTE_SCRIPT_PATH}.backup"
...
LOG_FILE="/home/ubuntu/audio_emotion/wav2vec_comma_fixed_${TIMESTAMP}.log"
...
cat > monitor_fixed_comma.sh << EOF
#!/bin/bash
...
echo "To monitor the training progress, run: ./monitor_fixed_comma.sh"

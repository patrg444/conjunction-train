#!/bin/bash
# Deploy and run the branched model with synchronized data augmentation

# Set up variables
INSTANCE_IP="3.235.76.0"
USERNAME="ec2-user"
KEY_FILE="../aws-setup/emotion-recognition-key-fixed-20250323090016.pem"
SCRIPT_NAME="train_branched_sync_aug.py"
LOCAL_DIR="$(pwd)"
REMOTE_DIR="~/emotion_training"
LOG_FILE="training_branched_sync_aug.log"

echo "==================================================="
echo "  DEPLOYING BRANCHED MODEL WITH SYNCHRONIZED AUGMENTATION"
echo "==================================================="
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "cat > ${REMOTE_DIR}/run_branched_sync_aug.sh << 'EOL'
#!/bin/bash
cd ~/emotion_training

# Run the synchronized augmentation script with Python 3.9
nohup /usr/bin/python3.9 scripts/${SCRIPT_NAME} > ${LOG_FILE} 2>&1 &
echo \$! > branched_sync_aug_pid.txt
echo \"Training process started with PID \$(cat branched_sync_aug_pid.txt)\"
...
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "chmod +x ${REMOTE_DIR}/run_branched_sync_aug.sh"

# Run the script
echo "Starting training process on remote server..."
ssh -i "${KEY_FILE}" ${USERNAME}@${INSTANCE_IP} "${REMOTE_DIR}/run_branched_sync_aug.sh"

echo
echo "Deployment complete!"
echo "To monitor training, run: cd aws-setup && ./monitor_branched_sync_aug.sh"
echo "==================================================="

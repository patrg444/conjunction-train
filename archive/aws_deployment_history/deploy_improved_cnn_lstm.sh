#!/bin/bash
# Deploy improved CNN-LSTM model with stronger regularization and attention
# Script to transfer the improved model with regularization to EC2 and start training
...
LOCAL_MODEL_FILE="improved_cnn_lstm_model.py"

echo -e "${BLUE}Preparing to deploy improved CNN-LSTM model with attention${NC}"
echo -e "${YELLOW}This model includes:${NC}"
echo -e "  - Attention mechanism"
...
echo -e "${BLUE}Transferring improved model to EC2...${NC}"
scp -i "$EC2_KEY" "$LOCAL_MODEL_FILE" "$EC2_INSTANCE:$REMOTE_DIR/improved_cnn_lstm_model.py"

# Make the script executable on the remote server
echo -e "${BLUE}Setting execution permissions...${NC}"
ssh -i "$EC2_KEY" "$EC2_INSTANCE" "chmod +x $REMOTE_DIR/improved_cnn_lstm_model.py"
...
echo -e "${BLUE}Starting improved CNN-LSTM training...${NC}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="improved_cnn_lstm_training_${TIMESTAMP}.log"

ssh -i "$EC2_KEY" "$EC2_INSTANCE" "
    cd $REMOTE_DIR
    screen -dmS improved_cnn_lstm bash -c \"
        echo 'Starting training at $(date)' > $LOG_FILE
        python3 improved_cnn_lstm_model.py 2>&1 | tee -a $LOG_FILE
        echo 'Training completed at $(date)' >> $LOG_FILE
    \"
"

echo -e "${GREEN}Improved CNN-LSTM model deployment complete!${NC}"
echo -e "${YELLOW}Training is running in a screen session on the EC2 instance.${NC}"
echo -e "Monitor training progress with: ${BLUE}./monitor_ec2_cnn_lstm_training.sh${NC}"
echo

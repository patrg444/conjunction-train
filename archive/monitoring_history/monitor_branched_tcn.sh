#!/bin/bash
# Monitor the branched model with TCN training on EC2
...
LOG_FILE="/home/ec2-user/emotion_training/training_branched_tcn.log"

echo "======================================================"
echo "  MONITORING BRANCHED MODEL WITH TCN"
echo "======================================================"
...
PROCESS_INFO=$(ssh -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "ps aux | grep train_branched_tcn.py | grep -v grep")

if [ -z "$PROCESS_INFO" ]; then
    echo "Warning: Training process is not running"
    echo "To start the training, use: ./aws-setup/deploy_branched_tcn.sh"
    exit 1
fi

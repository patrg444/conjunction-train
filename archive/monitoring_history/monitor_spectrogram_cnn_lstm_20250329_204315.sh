...
# Generated specifically for run started around: 20250329_204315
...
REMOTE_MONITOR_HELPER="monitor_helper_spectrogram_cnn_lstm_20250329_204315.sh" # Name of the helper script on EC2

echo -e "${BLUE}==================================================================${NC}"
echo -e "${GREEN}    MONITORING SPECTROGRAM CNN + LSTM MODEL TRAINING (20250329_204315)    ${NC}" # Updated Title
echo -e "${BLUE}==================================================================${NC}"
...
echo -e "${YELLOW}Setting correct permissions for the key file...${NC}"
chmod 400 "$KEY_FILE"
if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Failed to set permissions on key file: $KEY_FILE${NC}"
    exit 1
fi

#!/bin/bash
# Enhanced monitoring script for wav2vec fixed training
...
echo "================================================"
echo -e "${BLUE}Enhanced Wav2Vec Training Monitor${NC}"
echo "================================================"
...
check_training_status() {
    ssh -i ~/Downloads/gpu-key.pem ubuntu@54.162.134.77 "ps aux | grep train_wav2vec_audio_only_fixed | grep -v grep" > /dev/null
    if [ $? -eq 0 ]; then
...
LOG_FILE=$(ssh -i ~/Downloads/gpu-key.pem ubuntu@54.162.134.77 "ls -t /home/ubuntu/audio_emotion/wav2vec_fixed_training_*.log 2>/dev/null | head -1")

if [ -z "$LOG_FILE" ]; then
    echo -e "${RED}No training log file found.${NC}"
    echo "Has the training been started? Check if deploy_fixed_wav2vec_training.sh was run."
    exit 1
fi

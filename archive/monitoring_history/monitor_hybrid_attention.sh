#!/bin/bash
# Monitor the hybrid Conv1D-TCN model training on EC2
...
LOG_FILE="/home/ec2-user/emotion_training/hybrid_attention_training.log"

echo "======================================================"
echo "  MONITORING HYBRID CONV1D-TCN MODEL TRAINING"
echo "======================================================"
...
echo "Checking if the hybrid model training process is running..."
PROCESS_INFO=$(ssh -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "ps aux | grep train_hybrid_attention.py | grep -v grep")

if [ -z "$PROCESS_INFO" ]; then
    echo "Warning: Hybrid model training process is not running"
    echo "To start the training, use: ./aws-setup/deploy_hybrid_attention.sh"
    exit 1
fi

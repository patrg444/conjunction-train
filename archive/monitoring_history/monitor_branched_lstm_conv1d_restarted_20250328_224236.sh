#!/bin/bash
# Monitoring script for the RESTARTED Branched LSTM/Conv1D model training
# Generated specifically for run started around: 20250328_224236
...
LOG_FILE="training_branched_lstm_conv1d_20250328_224236.log" # Specific log file for this run
REMOTE_DIR="/home/ec2-user/emotion_training"
PID_FILE="branched_lstm_conv1d_20250328_224236_pid.txt" # Specific PID file for this run
MONITORING_INTERVAL=30

echo -e "${BLUE}==================================================================${NC}"
echo -e "${GREEN}    MONITORING RESTARTED BRANCHED LSTM/Conv1D (20250328_224236)    ${NC}"
echo -e "${BLUE}==================================================================${NC}"
...
ssh -t -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "
    ...
    if [ -n \"\$BEST_ACC_LINE\" ]; then
        ...
        echo \"\$BEST_ACC_LINE\"
         ...
         BEST_ACC_LINE_INIT=\$(grep 'Epoch 00001: val_accuracy improved from' \"${REMOTE_DIR}/${LOG_FILE}\" | head -n 1) # Check for Epoch 1 improvement if no others found
         if [ -n \"\$BEST_ACC_LINE_INIT\" ]; then
            echo -e '\033[1;33mInitial validation accuracy recorded:\033[0m'
            echo \"\$BEST_ACC_LINE_INIT\"
         else
            # Check if weights were loaded (indicating resume)
            if grep -q 'Weights loaded successfully' \"${REMOTE_DIR}/${LOG_FILE}\"; then
                echo -e '\033[1;33mWeights loaded, awaiting first validation improvement.\033[0m'
            else

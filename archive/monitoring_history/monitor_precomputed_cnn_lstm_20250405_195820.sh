...
# Generated specifically for run started around: 20250405_195820
...
REMOTE_MONITOR_HELPER="monitor_helper_precomputed_cnn_lstm_20250405_195820.sh" # Name of the helper script on EC2

echo -e "${BLUE}==================================================================${NC}"
echo -e "${GREEN} MONITORING PRECOMPUTED CNN AUDIO + LSTM MODEL TRAINING (20250405_195820) ${NC}" # Updated Title
echo -e "${BLUE}==================================================================${NC}"
echo -e "${YELLOW}Instance:${NC} $USERNAME@$INSTANCE_IP"
echo -e "${YELLOW}Executing remote helper:${NC} $REMOTE_DIR/$REMOTE_MONITOR_HELPER"
echo -e "${BLUE}==================================================================${NC}"

# Start continuous monitoring by executing the remote helper script using its full path
echo -e "${YELLOW}Starting continuous real-time monitoring... Press Ctrl+C to exit.${NC}"
ssh -t -i "$KEY_FILE" "$USERNAME@$INSTANCE_IP" "/home/ec2-user/emotion_training/" # Execute using full path

...
echo "=== Fixed Facenet Video-Only LSTM Training ==="
...
if [ "$LOCAL_RUN" = true ]; then
  ...
  ssh "$EC2_HOST" "chmod +x $REMOTE_TRAINING_SCRIPT"
...
while true; do
  ...
  ssh "\$EC2_HOST" "cd \$REMOTE_PATH && find models/facenet_lstm_fixed_* -type f -name 'best_model.keras' -exec ls -l {} \;"

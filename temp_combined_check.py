...
VARIANT: Increased Learning Rate
"""
...
LEARNING_RATE = 0.0007     # VARIANT: Increased from 0.0006
...
print("VARIANT: Increased Learning Rate to 0.0007")
...
def train_model():
    ...
    checkpoint = ModelCheckpoint(
        'best_model_tcn_large_fixed_v2_lr_increase.keras',
    ...
    model.load_weights('best_model_tcn_large_fixed_v2_lr_increase.keras')
    loss, accuracy = model.evaluate(val_generator, verbose=1)
    print("Best model validation accuracy: %.4f" % accuracy)

# --- Appended Code Block Starts Here ---

    # Create output directories if they don't exist
    model_dir = f'models/{MODEL_DIR_NAME_ARG}' # Use passed model dir name
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    # Define callbacks with more sophisticated setup
    checkpoint_path = os.path.join(model_dir, f'model_best_{MODEL_VERSION}.keras') # Use version in checkpoint name
    ...
    # Set up CSV logger for better tracking
    csv_logger = tf.keras.callbacks.CSVLogger(
        f'training_metrics_{MODEL_VERSION}.csv',
        separator=',',
        append=False
    )
    ...
    callbacks = [
        ...
        EarlyStopping(
            monitor='val_accuracy',
            patience=PATIENCE * 2, # Increased patience for early stopping
            restore_best_weights=True,
            mode='max',
            verbose=1
        ),
        ReduceLROnPlateau(
            monitor='val_loss',
            factor=0.6,         # Less aggressive reduction
            patience=PATIENCE,  # Use defined patience
            min_lr=5e-6,
            verbose=1,
            mode='min' # Monitor loss for LR reduction
        ),
        lr_scheduler,
        csv_logger,
    ...
    for i in range(NUM_CLASSES):
        ...
        # Avoid division by zero if a class has no samples in training set (unlikely but safe)
        class_weights[i] = total_samples / (NUM_CLASSES * class_count) if class_count > 0 else 1.0

    print('Using class weights to handle imbalance:', class_weights)
    print(f'Training with model version: {MODEL_VERSION}')
    ...
    class ValidationAccuracyCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            ...
            if val_acc is not None: # Check if val_accuracy exists
                with open(f'val_accuracy_log_{MODEL_VERSION}.txt', 'a') as f: # Use version in log name
                    f.write(f'Epoch {epoch+1}: val_accuracy={val_acc:.6f}\\n')
                print(f'Epoch {epoch+1} validation accuracy: {val_acc:.6f}')
            else:
                print(f'Epoch {epoch+1}: val_accuracy not available in logs.')

    # Add to callbacks
    callbacks.append(ValidationAccuracyCallback())
    ...
    final_model_path = os.path.join(model_dir, f'final_model_{MODEL_VERSION}.keras')
    model.save(final_model_path)
    print('Final model saved to:', final_model_path)

    # Save training history
    with open(f'training_history_{MODEL_VERSION}.json', 'w') as f:
        # Convert numpy types to native Python types for JSON serialization
        history_dict = {key: [float(v) for v in values] for key, values in history.history.items()}
        json.dump(history_dict, f, indent=4)
    print(f'Training history saved to: training_history_{MODEL_VERSION}.json')
    ...
    if 'val_accuracy' in history.history:
        ...
        # Add best validation accuracy to separate log for easy extraction
        with open(f'best_val_accuracy_{MODEL_VERSION}.txt', 'w') as f: # Use 'w' to overwrite/create per version
            f.write(f'{MODEL_VERSION},{best_val_acc:.6f},{best_epoch}\\n')
    if 'val_loss' in history.history:
        print('- Best validation loss:', min(history.history['val_loss']))

    # Ensure return is properly indented inside the function
    # return model, history # This line caused the original error, ensure it's correctly placed or removed if not needed at top level

if __name__ == '__main__':
    try:
        # Set the model version for this run (passed from shell script)
        MODEL_VERSION = os.environ.get("MODEL_VERSION_ENV", "unknown_version")
        MODEL_DIR_NAME_ARG = os.environ.get("MODEL_DIR_NAME_ENV", "unknown_model_dir")
        ...
        import tensorflow as tf
        from synchronized_data_generator import SynchronizedAugmentationDataGenerator # Ensure imports are within scope if needed
        from sequence_data_generator import ValidationDataGenerator # Ensure imports are within scope if needed
        ...
        # Train the model by calling the function defined in the base script
        # Assuming the base script defines a train_model() function

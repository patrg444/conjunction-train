#!/bin/bash
# Deploy and launch the fixed wav2vec training script (v4) on EC2
# This version uses Keras-compatible ops instead of raw TF ops for the attention mechanism
...
LOCAL_SCRIPT="./scripts/train_wav2vec_audio_only_fixed_v4.py"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="wav2vec_fixed_training_v4_${TIMESTAMP}.log"

echo "Deploying improved wav2vec training script with Keras compatibility fixes"
...
ssh -i $KEY_PATH $EC2_HOST "cd $REMOTE_DIR && nohup python3 $(basename $LOCAL_SCRIPT) \
  ...
  --lr=0.001 \
  --dropout=0.5 \
  --model_name=wav2vec_audio_only_fixed_v4_${TIMESTAMP} \
  --debug \
  > $LOG_FILE 2>&1 &"

echo "Training job started! Log file: $REMOTE_DIR/$LOG_FILE"
echo "To monitor training progress:"
echo "  ./monitor_wav2vec_fixed_training_v4.sh"
echo ""
